...
int
st25r_i2c_init(const struct device *dev)
{
...
bool
i2c_address_mismatch(uint16_t addr, const struct st25r_device_config *config)
{
    if ((addr >> 1) != config->i2c.addr) {
        LOG_ERR("I2C address mismatch: %#04x != %#04x", addr >> 1,
                config->i2c.addr);
        return true;
    }
    return false;
...
platform_st25r_i2c_send(uint16_t addr, uint8_t *txBuf, uint16_t len, bool last,
                        bool txOnly)
{
    LOG_DBG("platform_st25r_i2c_send addr=%#04x, len=%d, last=%d, txOnly=%d",
            addr >> 1, len, last, txOnly);
    ...
    if (last && !txOnly) {
        ...
        msgs[msg_count - 1].flags =
            I2C_MSG_WRITE | (last && txOnly ? I2C_MSG_STOP : 0);
        ...
        int res =
            i2c_transfer(config->i2c.bus, msgs, msg_count, config->i2c.addr);
        if (res < 0) {
