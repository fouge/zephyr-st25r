...
static void
st25r_handle_interrupt(const struct device *dev)
{
...
static void
st25r_gpio_callback(const struct device *dev, struct gpio_callback *cb,
                    uint32_t pins)
{
    struct st25r_data *st25r = CONTAINER_OF(cb, struct st25r_data, gpio_cb);
...
static void
st25r_thread(struct st25r_data *st25r)
{
    while (1) {
...
static void
st25r_work_cb(struct k_work *work)
{
    struct st25r_data *st25r = CONTAINER_OF(work, struct st25r_data, work);
...
int
st25r_init_interrupt(const struct device *dev)
{
    ...
    k_thread_create(&st25r->thread, st25r->thread_stack,
                    ...
                    (k_thread_entry_t)st25r_thread, st25r, 0, NULL,
                    K_PRIO_COOP(CONFIG_ST25R_THREAD_PRIORITY), 0, K_NO_WAIT);
#elif defined(CONFIG_ST25R_TRIGGER_GLOBAL_THREAD)
    ...
    gpio_init_callback(&st25r->gpio_cb, st25r_gpio_callback,
