 ...
 */
void
rfalDpoInitialize(void)
{
    /* Use the default Dynamic Power values */
    gRfalCurrentDpo = (uint8_t *)rfalDpoDefaultSettings;
    gRfalDpoTableEntries =
        (sizeof(rfalDpoDefaultSettings) / RFAL_DPO_TABLE_PARAMETER);
...
void
rfalDpoSetMeasureCallback(rfalDpoMeasureFunc pMeasureFunc)
{
...
/*******************************************************************************/
ReturnCode
rfalDpoTableWrite(rfalDpoEntry *powerTbl, uint8_t powerTblEntries)
{
    uint8_t entry = 0;

    /* check if the table size parameter is too big */
    if ((powerTblEntries * RFAL_DPO_TABLE_PARAMETER) >
        RFAL_DPO_TABLE_SIZE_MAX) {
        return ERR_NOMEM;
    }
...
/*******************************************************************************/
ReturnCode
rfalDpoTableRead(rfalDpoEntry *tblBuf, uint8_t tblBufEntries,
                 uint8_t *tableEntries)
{
    /* wrong request */
    if ((tblBuf == NULL) || (tblBufEntries < gRfalDpoTableEntries) ||
        (tableEntries == NULL)) {
        return ERR_PARAM;
    }

    /* Copy the whole Table to the given buffer */
    ST_MEMCPY(tblBuf, gRfalCurrentDpo,
              (tblBufEntries * RFAL_DPO_TABLE_PARAMETER));
...
rfalDpoAdjust(void)
{
    ...
    if (refValue >=
        dpoTable[gRfalDpoTableEntry].inc) { /* Increase the output power */
    ...
    } else if (refValue <= dpoTable[gRfalDpoTableEntry]
    ...
    /* Technology field is being extended for DPO: 2msb are used for treshold
     * step (only 4 allowed) */
    ...
    modeID |= ((gRfalDpoTableEntry << RFAL_DPO_ANALOGCONFIG_SHIFT) &
...
/*******************************************************************************/
rfalDpoEntry *
rfalDpoGetCurrentTableEntry(void)
{
    rfalDpoEntry *dpoTable = (rfalDpoEntry *)gRfalCurrentDpo;
