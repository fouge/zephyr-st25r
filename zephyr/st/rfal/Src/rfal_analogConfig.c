...
#if RFAL_FEATURE_DYNAMIC_ANALOG_CONFIG
static void
rfalAnalogConfigPtrUpdate(const uint8_t *analogConfigTbl);
#endif /* RFAL_FEATURE_DYNAMIC_ANALOG_CONFIG */
...
void
rfalAnalogConfigInitialize(void)
{
...
/* Check whether the Default Analog settings are to be used or custom ones */
    ...
    gRfalAnalogConfigMgmt.currentAnalogConfigTbl =
        rfalAnalogConfigCustomSettings;
    ...
    gRfalAnalogConfigMgmt.configTblSize =
        sizeof(rfalAnalogConfigDefaultSettings);
#endif
...
bool
rfalAnalogConfigIsReady(void)
{
    return gRfalAnalogConfigMgmt.ready;
}

ReturnCode
rfalAnalogConfigListWriteRaw(const uint8_t *configTbl, uint16_t configTblSize)
{
#if RFAL_FEATURE_DYNAMIC_ANALOG_CONFIG
    ...
    /* NOTE: Function does not check for the validity of the Table contents
     * (conf IDs, conf sets, register address)  */
...
ReturnCode
rfalAnalogConfigListWrite(uint8_t more, const rfalAnalogConfig *config)
{
#if RFAL_FEATURE_DYNAMIC_ANALOG_CONFIG
    ...
    if ((RFAL_ANALOG_CONFIG_TECH_RFU <=
        ...
        ((RFAL_ANALOG_CONFIG_BITRATE_6780 <
         ...
         (RFAL_ANALOG_CONFIG_BITRATE_53 >
          RFAL_ANALOG_CONFIG_ID_GET_BITRATE(configId))) ||
        (RFAL_ANALOG_CONFIG_BITRATE_1OF256 <
         RFAL_ANALOG_CONFIG_ID_GET_BITRATE(configId))) {
        rfalAnalogConfigInitialize(); /* Revert to default Analog Configuration
                                       */
        return ERR_PARAM;
     ...
     */
    ST_MEMCPY(&gRfalAnalogConfig[gRfalAnalogConfigMgmt.configTblSize],
...
ReturnCode
rfalAnalogConfigListReadRaw(uint8_t *tblBuf, uint16_t tblBufLen,
                            uint16_t *configTblSize)
{
    ...
    {
        ST_MEMCPY(tblBuf, gRfalAnalogConfigMgmt.currentAnalogConfigTbl,
                  gRfalAnalogConfigMgmt.configTblSize);
    }
...
ReturnCode
rfalAnalogConfigListRead(rfalAnalogConfigOffset *configOffset, uint8_t *more,
...
ReturnCode
rfalSetAnalogConfig(rfalAnalogConfigId configId)
{
    ...
    while (true) {
        ...
        configTbl =
            (rfalAnalogConfigRegAddrMaskVal *)((uint32_t)gRfalAnalogConfigMgmt
        ...
        for (i = 0; i < numConfigSet; i++) {
            if ((GETU16(configTbl[i].addr) & RFAL_TEST_REG) != 0U) {
                ...
                EXIT_ON_ERR(retCode, rfalChipChangeRegBits(
                                         GETU16(configTbl[i].addr),
                                         configTbl[i].mask, configTbl[i].val));
            }
        }
...
uint16_t
rfalAnalogConfigGenModeID(rfalMode md, rfalBitRate br, uint16_t dir)
{
    uint16_t id;
    ...
    /* Assign Bitrate */
    id |=
        (((((uint16_t)(br) >= (uint16_t)RFAL_BR_52p97) ? (uint16_t)(br)
                                                       : ((uint16_t)(br) + 1U))
...
rfalAnalogConfigSearch(rfalAnalogConfigId configId, uint16_t *configOffset)
{
    ...
    while (i < gRfalAnalogConfigMgmt.configTblSize) {
        ...
        i += (uint16_t)(sizeof(rfalAnalogConfigId) +
                        sizeof(rfalAnalogConfigNum) +
