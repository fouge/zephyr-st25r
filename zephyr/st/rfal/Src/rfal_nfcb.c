...
#define RFAL_FEATURE_NFCB                                                      \
    false /* NFC-B module configuration missing. Disabled by default */
#endif
...
typedef enum {
    RFAL_NFCB_CR_SLOTS_TX, /*!< State where slots are open and slot markers
                              issued  */
...
#define rfalNfcbNI2NumberOfSlots(ni)                                           \
    (uint8_t)(1U << (ni)) /*!< Converts the Number of slots Identifier to slot \
                             number */
...
*/
static ReturnCode
rfalNfcbCheckSensbRes(const rfalNfcbSensbRes *sensbRes, uint8_t sensbResLen);
static ReturnCode
rfalNfcbPollerSleepTx(const uint8_t *nfcid0);
...
rfalNfcbPollerSleepTx(const uint8_t *nfcid0)
{
    ...
    EXIT_ON_ERR(ret, rfalTransceiveBlockingTx((uint8_t *)&slpbReq,
                                              sizeof(rfalNfcbSlpbReq), NULL, 0,
                                              NULL, RFAL_TXRX_FLAGS_DEFAULT,
...
/*******************************************************************************/
ReturnCode
rfalNfcbPollerInitializeWithParams(uint8_t AFI, uint8_t PARAM)
{
    ReturnCode ret;
...
/*******************************************************************************/
ReturnCode
rfalNfcbPollerCheckPresence(rfalNfcbSensCmd cmd, rfalNfcbSlots slots,
                            rfalNfcbSensbRes *sensbRes, uint8_t *sensbResLen)
{
    ReturnCode ret;

    EXIT_ON_ERR(ret, rfalNfcbPollerStartCheckPresence(cmd, slots, sensbRes,
                                                      sensbResLen));
...
/*******************************************************************************/
ReturnCode
rfalNfcbPollerStartCheckPresence(rfalNfcbSensCmd cmd, rfalNfcbSlots slots,
                                 ...
                                 uint8_t *sensbResLen)
{
    ...
    *sensbResLen = 0;
    ...
    return rfalTransceiveBlockingTx(
        (uint8_t *)&sensbReq, sizeof(rfalNfcbSensbReq), (uint8_t *)sensbRes,
        sizeof(rfalNfcbSensbRes), &gRfalNfcb.DT.rxLen, RFAL_TXRX_FLAGS_DEFAULT,
        RFAL_NFCB_FWTSENSB);
}

/*******************************************************************************/
ReturnCode
rfalNfcbPollerGetCheckPresenceStatus(void)
{
    ReturnCode ret;
    ...
    if ((ret == ERR_CRC) || (ret == ERR_FRAMING)) {
        /* Invalidate received frame as an error was detected
         * (CollisionResolution checks if valid) */
...
/*******************************************************************************/
ReturnCode
rfalNfcbPollerSleep(const uint8_t *nfcid0)
{
    uint16_t rxLen;
    ReturnCode ret;
    rfalNfcbSlpbReq slpbReq;
...
/*******************************************************************************/
ReturnCode
rfalNfcbPollerSlotMarker(uint8_t slotCode, rfalNfcbSensbRes *sensbRes,
                         uint8_t *sensbResLen)
{
    ReturnCode ret;

    EXIT_ON_ERR(ret,
                rfalNfcbPollerStartSlotMarker(slotCode, sensbRes, sensbResLen));
...
/*******************************************************************************/
ReturnCode
rfalNfcbPollerStartSlotMarker(uint8_t slotCode, rfalNfcbSensbRes *sensbRes,
                              uint8_t *sensbResLen)
{
...
/*******************************************************************************/
ReturnCode
rfalNfcbPollerGetSlotMarkerStatus(void)
{
    ReturnCode ret;
...
/*******************************************************************************/
ReturnCode
rfalNfcbPollerStartTechnologyDetection(rfalComplianceMode compMode,
                                       rfalNfcbSensbRes *sensbRes,
                                       uint8_t *sensbResLen)
{
...
/*******************************************************************************/
ReturnCode
rfalNfcbPollerGetTechnologyDetectionStatus(void)
{
...
/*******************************************************************************/
ReturnCode
rfalNfcbPollerCollisionResolution(rfalComplianceMode compMode, uint8_t devLimit,
                                  rfalNfcbListenDevice *nfcbDevList,
                                  uint8_t *devCnt)
{
    ReturnCode ret;

    EXIT_ON_ERR(ret, rfalNfcbPollerStartCollisionResolution(
...
/*******************************************************************************/
ReturnCode
rfalNfcbPollerSlottedCollisionResolution(rfalComplianceMode compMode,
                                         uint8_t devLimit,
                                         rfalNfcbSlots initSlots,
                                         rfalNfcbSlots endSlots,
                                         rfalNfcbListenDevice *nfcbDevList,
                                         uint8_t *devCnt, bool *colPending)
{
    ReturnCode ret;

    EXIT_ON_ERR(ret, rfalNfcbPollerStartSlottedCollisionResolution(
                         compMode, devLimit, initSlots, endSlots, nfcbDevList,
                         devCnt, colPending));
    rfalRunBlocking(ret, rfalNfcbPollerGetCollisionResolutionStatus());
...
/*******************************************************************************/
ReturnCode
rfalNfcbPollerStartCollisionResolution(rfalComplianceMode compMode,
                                       uint8_t devLimit,
                                       rfalNfcbListenDevice *nfcbDevList,
                                       uint8_t *devCnt)
{
...
rfalNfcbPollerStartSlottedCollisionResolution(rfalComplianceMode compMode,
                                              ...
                                              uint8_t *devCnt, bool *colPending)
{
    /* Check parameters. In ISO | Activity 1.0 mode the initial slots must be 1
     * as continuation of Technology Detection */
...
/*******************************************************************************/
ReturnCode
rfalNfcbPollerGetCollisionResolutionStatus(void)
{
    ReturnCode ret;
    rfalNfcbSensCmd cmd;
    ...
    case RFAL_NFCB_CR_SLOTS_TX:
        ...
        if ((gRfalNfcb.CR.compMode == RFAL_COMPLIANCE_MODE_ISO) &&
            ...
            /* Initialise as no error in case Activity 1.0 where the previous
             * SENSB_RES from technology detection should be used */
            ret = ERR_NONE;
        }
        /* Send SENSB_REQ with number of slots if not the first
           Activity 2.1  9.3.5.24  -  Symbol 23 */
        else if (gRfalNfcb.CR.curSlotNum == 0U) {
            /* Send ALLB_REQ   Activity 2.1   9.3.5.2 and 9.3.5.3  (Symbol 1 and
             * 2) */
            cmd = ((gRfalNfcb.CR.curSlots == (uint8_t)RFAL_NFCB_SLOT_NUM_1)
                       ? RFAL_NFCB_SENS_CMD_ALLB_REQ
                       : RFAL_NFCB_SENS_CMD_SENSB_REQ);

            /* PRQA S 4342 1 # MISRA 10.5 - Layout of rfalNfcbSlots and the
             * limited loop guarantee that no invalid enum values are created.
             */
    ...
    case RFAL_NFCB_CR_SLOTS:
        ...
        if (gRfalNfcb.CR.compMode == RFAL_COMPLIANCE_MODE_EMV) {
            ...
            /* Check if there was a transmission error on WUPB
             * EMVCo 3.0  9.3.3.1 */
        ...
        if (ret != ERR_TIMEOUT) {
            ...
            if ((rfalNfcbCheckSensbRes(
                     &gRfalNfcb.CR.nfcbDevList[*gRfalNfcb.CR.devCnt].sensbRes,
                     gRfalNfcb.CR.nfcbDevList[*gRfalNfcb.CR.devCnt]
                         .sensbResLen) == ERR_NONE) &&
                (ret == ERR_NONE)) {
                ...
                if (gRfalNfcb.CR.compMode == RFAL_COMPLIANCE_MODE_EMV) {
                    ...
                    /* Activity 2.1  9.3.5.10 and 9.3.5.11  -  Symbol 9 and
                     * Symbol 11*/
                    if (gRfalNfcb.CR.curDevCnt != 0U) {
                        rfalNfcbPollerSleepTx(
                            gRfalNfcb.CR
                                .nfcbDevList[(*gRfalNfcb.CR.devCnt) -
                                             (uint8_t)1U]
                                .sensbRes.nfcid0);
                        gRfalNfcb.CR
                            .nfcbDevList[(*gRfalNfcb.CR.devCnt) - (uint8_t)1U]
                            .isSleep = true;
                ...
                /* If deviceLimit is set to 0 the NFC Forum Device is configured
                 * to perform collision detection only  Activity 1.0
                 * and 1.1  9.3.5.5  - Symbol 4 */
...
/*******************************************************************************/
uint32_t
rfalNfcbTR2ToFDT(uint8_t tr2Code)
{
    /*******************************************************************************/
    /* MISRA 8.9 An object should be defined at block scope if its identifier
    ...
    return (uint32_t)
        rfalNfcbTr2Table[(tr2Code & RFAL_NFCB_SENSB_RES_PROTO_TR2_MASK)];
}
