...
*/
static ReturnCode
rfalIso15693PhyVCDCode1Of4(const uint8_t data, uint8_t *outbuffer,
                           uint16_t maxOutBufLen, uint16_t *outBufLen);
static ReturnCode
rfalIso15693PhyVCDCode1Of256(const uint8_t data, uint8_t *outbuffer,
                             uint16_t maxOutBufLen, uint16_t *outBufLen);
    ...
    const struct iso15693StreamConfig **needed_stream_config)
{
    ...
    ST_MEMCPY((uint8_t *)&gIso15693PhyConfig, (const uint8_t *)config,
...
ReturnCode
rfalIso15693PhyGetConfiguration(rfalIso15693PhyConfig_t *config)
{
...
ReturnCode
rfalIso15693VCDCode(uint8_t *buffer, uint16_t length, bool sendCrc,
    ...
    *actOutBufSize = 0;
    ...
    if (ISO15693_VCD_CODING_1_4 == gIso15693PhyConfig.coding) {
        ...
        *subbit_total_length =
        ...
        if (*offset != 0U) {
            if (outBufSize < 64U) { /* 64 should be safe: enough a single byte
                                       data in 1of256 */
                return ERR_NOMEM;
            }
        } else {
            if (outBufSize <
                65U) { /* At beginning of a frame we need at least 65 bytes to
                          start: enough for sof + 1byte data in 1of256 */
                return ERR_NOMEM;
            }
        }
    }
        ...
        if ((0U == crc) && (length != 0U)) {
            crc = rfalCrcCalculateCcitt(
                ...
                ((picopassMode) ? (buffer + 1U)
                                : buffer), /* CMD byte is not taken into account
                                              in PicoPass mode */
                ((picopassMode) ? (length - 1U)
                                : length)); /* CMD byte is not taken into
                                               account in PicoPass mode */
...
ReturnCode
rfalIso15693VICCDecode(const uint8_t *inBuf, uint16_t inBufLen, uint8_t *outBuf,
                       uint16_t outBufLen, uint16_t *outBufPos,
                       uint16_t *bitsBeforeCol, uint16_t ignoreBits,
                       bool picopassMode)
{
    ReturnCode err = ERR_NONE;
    uint16_t crc;
    ...
    *outBufPos = 0;
    ...
    /* first check for valid SOF. Since it starts with 3 unmodulated pulses it
     * is 0x17. */
    ...
    *bitsBeforeCol = bp;
    ...
    if (*outBufPos > 2U) {
        ...
        crc = rfalCrcCalculateCcitt(((picopassMode) ? 0xE012U : 0xFFFFU),
                                    outBuf, *outBufPos - 2U);
        crc = (uint16_t)((picopassMode) ? crc : ~crc);
                             ...
                             uint16_t maxOutBufLen, uint16_t *outBufLen)
{
    ...
    *outBufLen = 0;
    ...
    for (a = 0; a < 64U; a++) {
        ...
        outbuf++;
        (*outBufLen)++;
        tmp -= 4U; /*  PRQA S 2911 # CERT INT30 - Intentional underflow, part of
                      the coding */
    }
