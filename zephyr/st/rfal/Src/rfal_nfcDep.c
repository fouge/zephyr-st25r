...
#define NFCIP_TIMEOUT_ADJUSTMENT                                               \
    3072U /*!< Timeout Adjustment to compensate timing from end of Tx to end   \
             of frame  */
...
#define nfcip_PFBIPDU_MI(pni)                                                  \
    ((uint8_t)(isoDep_PCBIBlock(pni) |                                         \
               NFCIP_PFB_MI_BIT)) /*!< Returns a PFB I-PDU with the given      \
                                     packet number (pni) indicating chaing */
...
#define nfcip_PNIDec(pni)                                                      \
    ((uint8_t)(((pni)-1U) &                                                    \
               NFCIP_PFB_PNI_MASK)) /*!< Returns a decremented PNI from the    \
                                       given (pni)        */
...
#define nfcip_RTOXTargMax(wt)                                                  \
    (uint8_t)(MIN((RFAL_NFCDEP_RWT_TRG_MAX / rfalNfcDepWT2RWT(wt)),            \
...
#define nfcipIsEmptyDEPDisabled(op)                                            \
    (((op)&RFAL_NFCDEP_OPER_EMPTY_DEP_DIS) !=                                  \
     0U) /*!< Checks if empty payload is not allowed by operation config       \
            NCI 1.0 Table 81   */
...
#define nfcipRWTActivation()                                                   \
    ((gNfcip.cfg.commMode == RFAL_NFCDEP_COMM_ACTIVE)                          \
         ? NFCIP_RWT_ACM_ACTIVATION                                            \
...
typedef struct {
    ...
    uint8_t
        rxBufPaylPos; /*!< Position inside rxBuf where data is to be placed*/
...
static ReturnCode
nfcipTxRx(rfalNfcDepCmd cmd, uint8_t *txBuf, uint32_t fwt, uint8_t *paylBuf,
          uint8_t paylBufLen, uint8_t *rxBuf, uint16_t rxBufLen,
          uint16_t *rxActLen);
static ReturnCode
nfcipTx(rfalNfcDepCmd cmd, uint8_t *txBuf, uint8_t *paylBuf, uint16_t paylLen,
        uint8_t pfbData, uint32_t fwt);
static ReturnCode
nfcipDEPControlMsg(uint8_t pfb, uint8_t RTOX);
static ReturnCode
nfcipInitiatorHandleDEP(ReturnCode rxRes, uint16_t rxLen, uint16_t *outActRxLen,
                        bool *outIsChaining);
static ReturnCode
nfcipTargetHandleRX(ReturnCode rxRes, uint16_t *outActRxLen,
                    bool *outIsChaining);
static ReturnCode
nfcipTargetHandleActivation(rfalNfcDepDevice *nfcDepDev, uint8_t *outBRS);
 ...
 */
static void
nfcipConfig(const rfalNfcDepConfigs *cfg);
 ...
 */
static void
nfcipSetDEPParams(const rfalNfcDepDEPParams *DEPParams);
 ...
 */
static ReturnCode
nfcipRun(uint16_t *outActRxLen, bool *outIsChaining);
 ...
 */
static ReturnCode
nfcipDataTx(uint8_t *txBuf, uint16_t txBufLen, uint32_t fwt);
...
/*******************************************************************************/
static bool
nfcipDxIsSupported(uint8_t Dx, uint8_t BRx, uint8_t BSx)
{
    uint8_t Bx;
...
nfcipDEPControlMsg(uint8_t pfb, uint8_t RTOX)
{
    ...
    const rfalNfcDepCmd depCmd =
        ((gNfcip.cfg.role == RFAL_NFCDEP_ROLE_TARGET) ? NFCIP_CMD_DEP_RES
                                                      : NFCIP_CMD_DEP_REQ);
    fwt = ((gNfcip.cfg.role == RFAL_NFCDEP_ROLE_TARGET)
               ? NFCIP_NO_FWT
               : (nfcip_PFBisSTO(pfb)
                      ? ((RTOX * gNfcip.cfg.fwt) + gNfcip.cfg.dFwt)
                        ...
                        bool *outIsChaining)
{
    ...
    *outIsChaining = false;
    ...
    case ERR_TIMEOUT:
        ...
        /*******************************************************************************/
        /* Upon Timeout error, if Deactivation is pending, no more error
         * recovery will be done #54. This is used to address the issue some
         * devices that havea big TO. Normally LLCP layer has timeout already,
         * and NFCIP layer is still running error handling, retrying ATN/NACKs
         */
        /*******************************************************************************/
    ...
    if (nfcip_PFBisRPDU(rxPFB)) {
        ...
        if (nfcip_PFBisRACK(rxPFB)) {
                      ...
                      retransmit */
            {
                ...
                /* Extended the MAY in Digital 1.0 14.12.4.5 to only reTransmit
                 * if the ACK is for the previous DEP, otherwise raise Protocol
                 * immediately If the PNI difference is more than 1 it is
                 * worthless to reTransmit 3x and after raise the error */
                ...
                nfcipLogI(
                    " NFCIP(I) Rcvd ACK unexpected far PNI -> Error \r\n");
                return ERR_PROTO;
            }
    ...
    if (nfcip_PFBisSPDU(rxPFB)) {
        ...
        if (nfcip_PFBisSATN(rxPFB)) /* If is a S-ATN        */
        {
            ...
            if (nfcip_PFBisSATN(gNfcip.lastPFB)) /* Check if is expected */
            {
                ...
                /* Although spec is not clear NFC Forum Digital test is
                 * expecting to retransmit upon receiving ATN_RES */
                if (nfcip_PFBisSTO(gNfcip.lastPFBnATN)) {
                    nfcipLogI(" NFCIP(I) Rcvd ATN  -> reTx RTOX_RES \r\n");
                    EXIT_ON_ERR(ret, nfcipDEPControlMsg(nfcip_PFBSPDU_TO(),
                                                        gNfcip.lastRTOX));
                } else {
                    /* ReTransmit ? */
                    if (gNfcip.cntTxRetrys++ >= RFAL_NFCDEP_MAX_TX_RETRYS) {
        ...
        else if (nfcip_PFBisSTO(rxPFB)) /* If is a S-TO (RTOX)  */
        {
            ...
            /* Digital 1.1 16.12.4.3 - Initiator MAY stop accepting subsequent
             * RTOX Req   *
            ...
            if ((rxRTOX < NFCIP_INIT_MIN_RTOX) ||
                (rxRTOX > NFCIP_INIT_MAX_RTOX)) {
                return ERR_PROTO;
            }
    ...
    if (nfcip_PFBisIPDU(rxPFB)) {
        ...
        nfcipClearCounters();
        *outActRxLen = ((uint16_t)nfcDepLen - RFAL_NFCDEP_DEP_HEADER -
                        (uint16_t)optHdrLen);

        if ((&gNfcip.rxBuf[gNfcip.rxBufPaylPos] !=
             &gNfcip.rxBuf[RFAL_NFCDEP_DEP_HEADER + optHdrLen]) &&
            (*outActRxLen > 0U)) {
            ST_MEMMOVE(&gNfcip.rxBuf[gNfcip.rxBufPaylPos],
                       &gNfcip.rxBuf[RFAL_NFCDEP_DEP_HEADER + optHdrLen],
                       *outActRxLen);
        }
        ...
        if (nfcip_PFBisIMI(rxPFB)) {
            ...
            *outIsChaining = true;
            ...
            EXIT_ON_ERR(ret, nfcipDEPControlMsg(nfcip_PFBRPDU_ACK(gNfcip.pni),
                    ...
                    bool *outIsChaining)
{
    ...
    *outIsChaining = false;
    ...
    case ERR_PROTO:
    default:
        /* Digital 1.1  16.12.5.2 The Target MUST NOT attempt any error
         * recovery.      * The Target MUST always stay in receive mode when a *
         * Transmission Error or a Protocol Error occurs. *
         *                                                                             *
         * Do not push Transmission/Protocol Errors to upper layer in Listen
         * Mode #766 */
    ...
    case (uint8_t)NFCIP_CMD_DSL_REQ:
        ...
        /* [Digital 1.0, 16.9.1.2]: If DID == 0, Target SHALL ignore DSL_REQ
         * with DID */
        if ((((gNfcip.rxBuf[rxMsgIt++] != gNfcip.cfg.did) ||
    ...
    case (uint8_t)NFCIP_CMD_RLS_REQ:
        ...
        /* [Digital 1.0, 16.10.2.2]: If DID == 0, Target SHALL ignore DSL_REQ
         * with DID */
        if ((((gNfcip.rxBuf[rxMsgIt++] != gNfcip.cfg.did) ||
              (nfcDepLen != RFAL_NFCDEP_DSL_RLS_LEN_DID)) &&
             (gNfcip.cfg.did != RFAL_NFCDEP_DID_NO)) ||
    ...
    if (nfcip_PFBisRPDU(rxPFB)) {
        ...
        if (nfcip_PFBisRACK(rxPFB)) {
            ...
            if (gNfcip.pni == nfcip_PBF_PNI(rxPFB)) {
                /* R-ACK while not performing chaining -> Protocol error */
                if (!gNfcip.isTxChaining) {
                    nfcDepReEnableRx(gNfcip.rxBuf, gNfcip.rxBufLen,
                                     gNfcip.rxRcvdLen);
                    return ERR_BUSY; /* ERR_PROTO - Ignore unexpected ACK  */
                }

                /* This block has been transmitted and acknowledged, perform
                 * RTOX until next data is provided  */

                /* Digital 1.1  16.12.4.7 - If ACK rcvd continue with chaining
                 * or an RTOX */
                nfcipTimerStart(gNfcip.RTOXTimer,
                                nfcipRTOXAdjust(nfcipConv1FcToMs(
            ...
            /* Digital 1.0 14.12.3.4 - If last send was ATN and rx PNI is minus
               1 */
                ...
                /* Spec says to leave current PNI as is, but will be Inc after
                 * Tx, remaining the same */
        ...
        /* ISO 18092 12.6.1.3.3 When rcv NACK if PNI = prev PNI sent ->  reTx */
            ...
            gNfcip.pni =
                nfcip_PNIDec(gNfcip.pni); /* Dec so that has the prev PNI */
    ...
    if (nfcip_PFBisSPDU(rxPFB)) {
        ...
        else if (nfcip_PFBisSTO(rxPFB)) /* If is a S-TO (RTOX)  */
        {
            if (nfcip_PFBisSTO(gNfcip.lastPFBnATN)) {
                ...
                /* Digital 1.1  16.8.4.6  RTOX value in RES different that in
                 * REQ -> Protocol Error */
                ...
                if (gNfcip.isTxPending) {
                    nfcipLogW(
                        " NFCIP(T) Tx pending, go immediately to TX \r\n");
                ...
                nfcipTimerStart(
                    gNfcip.RTOXTimer,
                    nfcipRTOXAdjust(nfcipConv1FcToMs(
                        gNfcip.lastRTOX * rfalNfcDepWT2RWT(gNfcip.cfg.to))));
                gNfcip.state = NFCIP_ST_TARG_DEP_RTOX;
    ...
    if (nfcip_PFBisIPDU(rxPFB)) {
        if (gNfcip.pni != nfcip_PBF_PNI(rxPFB)) {
            ...
            /* Digital 1.1 16.12.3.4 - If last send was ATN and rx PNI is minus
             * 1 */
            if (nfcip_PFBisSATN(gNfcip.lastPFB) &&
                (nfcip_PNIDec(gNfcip.pni) == nfcip_PBF_PNI(rxPFB))) {
                /* Spec says to leave current PNI as is, but will be Inc after
                 * Data Tx, remaining the same */
                gNfcip.pni = nfcip_PNIDec(gNfcip.pni);

                if (nfcip_PFBisIMI(rxPFB)) {
                    nfcipLogI(" NFCIP(T) PNI = prevPNI && ATN before && "
                              "chaining -> send ACK  \r\n");
                    EXIT_ON_ERR(
                        ret, nfcipDEPControlMsg(nfcip_PFBRPDU_ACK(gNfcip.pni),
                                                gNfcip.rxBuf[rxMsgIt++]));

                    /* Digital 1.1 16.12.3.4 (...) leave the current PNI
                     * unchanged afterwards */
                    gNfcip.pni = nfcip_PNIInc(gNfcip.pni);
                } else {
                    nfcipLogI(" NFCIP(T) PNI = prevPNI && ATN before -> reTx "
                              "last I-PDU  \r\n");
                    gNfcip.state = NFCIP_ST_TARG_DEP_TX;
                }
        ...
        uint8_t pfbData, uint32_t fwt)
{
    ...
    if ((paylLen == 0U) || (payloadBuf == NULL)) {
        payloadBuf = (uint8_t *)&txBuf
            [RFAL_NFCDEP_DEPREQ_HEADER_LEN]; /* If not a DEP (no Data) ensure
                                                enough space for header */
    }
    ...
    case NFCIP_CMD_ATR_REQ:
        ...
        if (gNfcip.cfg.gbLen > 0U) {
            ...
            ST_MEMCPY(&payloadBuf[txBufIt], gNfcip.cfg.gb,
    ...
    *(--txBlock) =
    ...
    txBufIt +=
        paylLen +
        (uint16_t)((uint32_t)payloadBuf -
                   (uint32_t)txBlock); /* Calculate overall buffer size */

    if (txBufIt > gNfcip.fsc) /* Check if msg length violates the maximum
                                 payload size FSC */
    {
        return ERR_NOTSUPP;
    }
...
nfcipRun(uint16_t *outActRxLen, bool *outIsChaining)
{
    ...
    case NFCIP_ST_TARG_DEP_RTOX:
        ...
        if (nfcipIsRTOXReqDisabled(gNfcip.cfg.oper)) {
            ...
            nfcipLogI(" NFCIP(T) RTOX not sent due to config, NOT reenabling "
                      "Rx \r\n");
            return ERR_TIMEOUT;
        }
    ...
    case NFCIP_ST_TARG_DEP_RX: /*  PRQA S 2003 # MISRA 16.3 - Intentional fall
        ...
        if (gNfcip.isReqPending) /* if already has Data should be from a DEP
                                    from nfcipTargetHandleActivation()  */
        {
...
/*******************************************************************************/
void
rfalNfcDepSetDeactivatingCallback(rfalNfcDepDeactCallback pFunc)
{
    ...
    if (gNfcip.cfg.role == RFAL_NFCDEP_ROLE_TARGET) {
        ...
        if (DEPParams->txBufLen > 0U) {
            ...
            if (!gNfcip.isWait4RTOX) {
                ...
                nfcipLogW(
                    " NFCIP(T) Waiting RTOX, queueing outgoing DEP Block \r\n");
            }
        }

        /*Digital 1.0  14.12.4.1 In target mode the first PDU MUST be sent by
         * the Initiator */
...
/*******************************************************************************/
bool
rfalNfcDepIsAtrReq(const uint8_t *buf, uint16_t bufLen, uint8_t *nfcid3)
{
    uint8_t msgIt;
    ...
    if (nfcid3 != NULL) {
        ST_MEMCPY(nfcid3, &buf[RFAL_NFCDEP_ATR_REQ_NFCID3_POS],
    ...
    if (gNfcip.rxBuf[msgIt] == (uint8_t)NFCIP_CMD_PSL_REQ) {
        ...
        *outBRS = gNfcip.rxBuf[msgIt++]; /* assign output BRS value      */
        ...
        if (nfcDepDev != NULL) {
             ...
             * definition of rfalNfcDepBRS2DSI guarantee no invalid enum values
             * to be created */
        ...
        if (gNfcip.rxBuf[msgIt] == (uint8_t)NFCIP_CMD_DEP_REQ) {
            ...
            /*******************************************************************************/
            /* Digital 1.0 14.8.2.1  check if DID is expected and match ->
             * Protocol Error  */
...
/*******************************************************************************/
ReturnCode
rfalNfcDepATR(const rfalNfcDepAtrParam *param, rfalNfcDepAtrRes *atrRes,
              uint8_t *atrResLen)
{
    ReturnCode ret;
    ...
    EXIT_ON_ERR(ret, nfcipTxRx(NFCIP_CMD_ATR_REQ, txBuf, nfcipRWTActivation(),
                               NULL, 0, rxBuf, NFCIP_ATRRES_BUF_LEN, &rxLen));
...
rfalNfcDepPSL(uint8_t BRS, uint8_t FSL)
{
    ...
    /*******************************************************************************/
    EXIT_ON_ERR(ret,
                nfcipTxRx(NFCIP_CMD_PSL_REQ, txBuf,
                          (gNfcip.cfg.fwt + gNfcip.cfg.dFwt),
                          &txBuf[NFCIP_PSLREQ_LEN], (msgIt - NFCIP_PSLREQ_LEN),
                          rxBuf, NFCIP_PSLRES_LEN, &rxLen));
...
rfalNfcDepDSL(void)
{
    ...
    EXIT_ON_ERR(ret, nfcipTxRx(NFCIP_CMD_DSL_REQ, txBuf,
    ...
    if (rxBuf[rxMsgIt++] <
        NFCIP_DSLRES_MIN) /* Checking length: LEN + DSL_RES */
    {
        return ERR_PROTO;
    }
...
rfalNfcDepRLS(void)
{
    ...
    EXIT_ON_ERR(ret, nfcipTxRx(NFCIP_CMD_RLS_REQ, txBuf,
                               (gNfcip.cfg.fwt + gNfcip.cfg.dFwt), NULL, 0,
                               rxBuf, (uint16_t)sizeof(rxBuf), &rxLen));
    ...
    if (rxBuf[rxMsgIt++] <
        NFCIP_RLSRES_MIN) /* Checking length: LEN + RLS_RES */
    {
        return ERR_PROTO;
    }
...
/*******************************************************************************/
ReturnCode
rfalNfcDepInitiatorHandleActivation(rfalNfcDepAtrParam *param,
                                    ...
                                    rfalNfcDepDevice *nfcDepDev)
{
    ...
    param->NAD = RFAL_NFCDEP_NAD_NO; /* Digital 1.1  16.6.2.9  Initiator SHALL
                                        NOT use NAD */
    ...
    nfcDepDev->info.NAD =
        RFAL_NFCDEP_NAD_NO; /* Digital 1.1  16.6.3.11 Initiator SHALL ignore b1
                               of PPt */
    nfcDepDev->info.LR =
    ...
    if (sendPSL) {
        ...
        if (nfcDepDev->info.DSI != desiredBR) {
            /* Check if device was in Passive NFC-A and went to higher bit
             * rates, use NFC-F */
...
/*******************************************************************************/
uint32_t
rfalNfcDepCalculateRWT(uint8_t wt)
{
    ...
    if (ret != ERR_BUSY) {
        if (gNfcip.rxRcvdLen != NULL) {
            ...
            if ((ret == ERR_NONE) && (gNfcip.rxBuf != NULL)) {
                /* Digital 1.1  16.4.1.3 - Length byte LEN SHALL have a value
                 * between 3 and 255 -> otherwise treat as Transmission Error *
                 *                       - Ensure that actual received and frame
                 * length do match, otherwise treat as Transmission error  */
...
/*******************************************************************************/
ReturnCode
rfalNfcDepListenStartActivation(const rfalNfcDepTargetParam *param,
                                const uint8_t *atrReq, uint16_t atrReqLength,
                                rfalNfcDepListenActvParam rxParam)
{
    ...
    rxParam.nfcDepDev->activation.Initiator.ATR_REQLen =
        (uint8_t)atrReqLength; /* nfcipIsAtrReq() is already checking Min and
                                  Max buffer lengths */
    if (atrReqLength > 0U)     /* MISRA 21.18 */
    {
        ST_MEMCPY((uint8_t *)&rxParam.nfcDepDev->activation.Initiator.ATR_REQ,
                  atrReq, atrReqLength);
    }
    ...
    rxParam.nfcDepDev->info.NAD =
        RFAL_NFCDEP_NAD_NO; /* Digital 1.1  16.6.2.9  Initiator SHALL NOT use
                               NAD */
    ...
    EXIT_ON_ERR(ret, nfcipTx(NFCIP_CMD_ATR_RES, (uint8_t *)gNfcip.rxBuf, NULL,
                             0, 0, NFCIP_NO_FWT));
...
/*******************************************************************************/
ReturnCode
rfalNfcDepListenGetActivationStatus(void)
{
    ReturnCode err;
    uint8_t BRS;
    ...
    case ERR_NONE:
        ...
        if (BRS != RFAL_NFCDEP_BRS_MAINTAIN) {
            ...
            if (gNfcip.cfg.commMode == RFAL_NFCDEP_COMM_ACTIVE) {
                EXIT_ON_ERR(err, rfalSetMode(RFAL_MODE_LISTEN_ACTIVE_P2P,
                                             gNfcip.nfcDepDev->info.DRI,
                                             gNfcip.nfcDepDev->info.DSI));
            } else {
                EXIT_ON_ERR(err, rfalSetMode(((RFAL_BR_106 ==
                                               gNfcip.nfcDepDev->info.DRI)
...
/*******************************************************************************/
ReturnCode
rfalNfcDepStartTransceive(const rfalNfcDepTxRxParam *param)
{
    ...
    nfcDepParams.txBufPaylPos =
        RFAL_NFCDEP_DEPREQ_HEADER_LEN; /* position in txBuf where actual
                                          outgoing data is located */
...
/*******************************************************************************/
ReturnCode
rfalNfcDepGetTransceiveStatus(void)
{
...
/*******************************************************************************/
static void
rfalNfcDepPdu2BLockParam(rfalNfcDepPduTxRxParam pduParam,
                         rfalNfcDepTxRxParam *blockParam, uint16_t txPos,
                         uint16_t rxPos)
{
    uint16_t maxInfLen;
    ...
    blockParam->rxBuf =
        pduParam.tmpBuf; /* Simply using the pdu buffer is not possible because
...
/*******************************************************************************/
ReturnCode
rfalNfcDepStartPduTransceive(rfalNfcDepPduTxRxParam param)
{
...
/*******************************************************************************/
ReturnCode
rfalNfcDepGetPduTransceiveStatus(void)
{
    ReturnCode ret;
    rfalNfcDepTxRxParam txRxParam;
    ...
    case ERR_NONE:
        ...
        if (gNfcip.isTxChaining) {
            ...
            rfalNfcDepPdu2BLockParam(gNfcip.PDUParam, &txRxParam,
                                     gNfcip.PDUTxPos, gNfcip.PDURxPos);

            if (txRxParam.txBufLen > 0U) /* MISRA 21.18 */
            {
                /* Move next Block to beginning of APDU Tx buffer */
                ST_MEMCPY(gNfcip.PDUParam.txBuf->pdu,
    ...
    case ERR_AGAIN: /*  PRQA S 2003 # MISRA 16.3 - Intentional fall through */
        ...
        if ((*gNfcip.PDUParam.rxLen) > 0U) /* MISRA 21.18 */
        {
            ...
            if ((uint16_t)((uint16_t)gNfcip.PDURxPos +
            ...
            ST_MEMCPY(&gNfcip.PDUParam.rxBuf->pdu[gNfcip.PDURxPos],
