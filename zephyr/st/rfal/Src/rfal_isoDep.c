...
#define ISODEP_PCB_NAD_BIT                                                     \
    (0x04U) /*!< Bit mask for the NAD presence bit of an ISO DEP I,S,R Blocks  \
               in PCB  */
...
#define ISODEP_SWTX_WTXM_MASK                                                  \
    (0x3FU) /*!< Bit mask for the WTXM bits of the inf byte of an WTX request  \
               or response        */
...
#define ISODEP_PCB_SWTX                                                        \
    (ISODEP_PCBSBLOCK |                                                        \
     ISODEP_PCB_WTX) /*!< PCB Value of a S-Block with WTX        */
...
#define RFAL_ISODEP_SPARAM_HDR_LEN                                             \
    (RFAL_ISODEP_PCB_LEN +                                                     \
     RFAL_ISODEP_SPARAM_TVL_HDR_LEN) /*!< S(PARAMETERS) header length: PCB +   \
                                        Tag + Len */
...
#define RFAL_ISODEP_ATS_TC_ADV_FEAT                                            \
    (0x10U) /*!< Bit mask indicating support for Advanced protocol features:   \
               DID & NAD */
...
#define rfalIsoDep_PCBIBlockChaining(bn)                                       \
    ((uint8_t)(rfalIsoDep_PCBIBlock(bn) |                                      \
               ISODEP_PCB_CHAINING_BIT)) /*!< Returns an I-Block with the      \
                                            given block number (bn) indicating \
                                            chaining */
...
#define rfalIsoDep_WTXMListenerMax(fwt)                                        \
    (MIN((uint8_t)(ISODEP_FWT_LIS_MAX / (fwt)),                                \
...
#define rfalIsoDepCalcSGFT(s)                                                  \
    (4096U * ((uint32_t)1U << (s))) /*!< Calculates the SFGT with given SFGI   \
                                       Digital 1.1  13.8.2         */
...
#define rfalIsoDep_ToggleBN(bn)                                                \
    ((bn) =                                                                    \
         (((bn) ^ 0x01U) & ISODEP_PCB_BN_MASK)) /*!< Toggles the block number  \
                                                   value of the given bn */

#define rfalIsoDep_WTXAdjust(v)                                                \
    ((v) - ((v) >> 3)) /*!< Adjust WTX timer value to a percentage of the      \
                          total, current 88% */
 ...
 */
static void
rfalIsoDepClearCounters(void);
static ReturnCode
rfalIsoDepTx(uint8_t pcb, const uint8_t *txBuf, uint8_t *infBuf,
             uint16_t infLen, uint32_t fwt);
static ReturnCode
rfalIsoDepHandleControlMsg(rfalIsoDepControlMsg controlMsg, uint8_t param);
static void
rfalIsoDepApdu2IBLockParam(rfalIsoDepApduTxRxParam apduParam,
                           rfalIsoDepTxRxParam *iBlockParam, uint16_t txPos,
                           uint16_t rxPos);

#if RFAL_FEATURE_ISO_DEP_POLL
static ReturnCode
rfalIsoDepDataExchangePCD(uint16_t *outActRxLen, bool *outIsChaining);
static void
rfalIsoDepCalcBitRate(rfalBitRate maxAllowedBR, uint8_t piccBRCapability,
                      rfalBitRate *dsi, rfalBitRate *dri);
static uint32_t
rfalIsoDepSFGI2SFGT(uint8_t sfgi);

#if RFAL_FEATURE_NFCA
static ReturnCode
rfalIsoDepStartRATS(rfalIsoDepFSxI FSDI, uint8_t DID, rfalIsoDepAts *ats,
                    uint8_t *atsLen);
static ReturnCode
rfalIsoDepGetRATSStatus(void);
static ReturnCode
rfalIsoDepStartPPS(uint8_t DID, rfalBitRate DSI, rfalBitRate DRI,
                   rfalIsoDepPpsRes *ppsRes);
static ReturnCode
rfalIsoDepGetPPSSTatus(void);
#endif /* RFAL_FEATURE_NFCA */

#if RFAL_FEATURE_NFCB
static ReturnCode
rfalIsoDepStartATTRIB(const uint8_t *nfcid0, uint8_t PARAM1, rfalBitRate DSI,
                      rfalBitRate DRI, rfalIsoDepFSxI FSDI, uint8_t PARAM3,
                      uint8_t DID, const uint8_t *HLInfo, uint8_t HLInfoLen,
                      uint32_t fwt, rfalIsoDepAttribRes *attribRes,
                      uint8_t *attribResLen);
static ReturnCode
rfalIsoDepGetATTRIBStatus(void);
#endif /* RFAL_FEATURE_NFCB */
...
static ReturnCode
rfalIsoDepDataExchangePICC(void);
static ReturnCode
rfalIsoDepReSendControlMsg(void);
#endif
             ...
             uint16_t infLen, uint32_t fwt)
{
    ...
    if (infLen > 0U) {
        if (((uint32_t)infBuf - (uint32_t)txBuf) <
            gIsoDep.hdrLen) /* Check that we can fit the header in the given
                               space */
        {
            return ERR_NOMEM;
        }
...
rfalIsoDepHandleControlMsg(rfalIsoDepControlMsg controlMsg, uint8_t param)
{
    ...
    case ISODEP_R_ACK:
        ...
        if (gIsoDep.cntRRetrys++ > gIsoDep.maxRetriesR) {
            return ERR_TIMEOUT; /* NFC Forum mandates timeout or transmission
...
/*******************************************************************************/
void
rfalIsoDepInitializeWithParams(rfalComplianceMode compMode, uint8_t maxRetriesR,
                               uint8_t maxRetriesSnWTX, uint8_t maxRetriesSWTX,
...
rfalIsoDepDataExchangePCD(uint16_t *outActRxLen, bool *outIsChaining)
{
    ...
    *outIsChaining = false;
    ...
    /* Check if there is enough space before the infPos to append ISO-DEP
     * headers on rx and tx */
    ...
    case ISODEP_ST_PCD_RX:
        ...
        case ERR_CRC:
        case ERR_PAR:
        case ERR_FRAMING:         /* added to handle test cases scenario
                                     TC_POL_NFCB_T4AT_BI_82_x_y &
                                     TC_POL_NFCB_T4BT_BI_82_x_y */
        case ERR_INCOMPLETE_BYTE: /* added to handle test cases scenario
                                     TC_POL_NFCB_T4AT_BI_82_x_y &
                                     TC_POL_NFCB_T4BT_BI_82_x_y */

            if (gIsoDep.isRxChaining) { /* Rule 5 - In PICC chaining when a
                                           invalid/timeout occurs -> R-ACK */
                EXIT_ON_ERR(ret, rfalIsoDepHandleControlMsg(
            ...
            } else { /* Rule 4 - When a invalid block or timeout occurs ->
                        R-NACK */
                EXIT_ON_ERR(ret, rfalIsoDepHandleControlMsg(
        ...
        if (rfalIsoDep_PCBisSBlock(rxPCB)) {
            ...
            if (rfalIsoDep_PCBisSWTX(rxPCB)) {
                ...
                /* Rule 3 - respond to S-block: get 1st INF byte S(STW): Power +
                 * WTXM */
                EXIT_ON_ERR(
                    ret, rfalIsoDepHandleControlMsg(
                             ISODEP_S_WTX, rfalIsoDep_GetWTXM(
        ...
        /*******************************************************************************/
            ...
            if (rfalIsoDep_PCBisRACK(rxPCB)) /* Check if is a R-ACK */
            {
                    ...
                    gIsoDep.blockNumber) /* Expected block number  */
                {
                    ...
                    /* EMVCo 3.0 10.3.4.3 -  PCD may re-transmit the last
                     * I-Block or report error */
                    if (gIsoDep.cntIRetrys++ < gIsoDep.maxRetriesI) {
                        gIsoDep.cntRRetrys = 0; /* Clear R counter only */
                        gIsoDep.state = ISODEP_ST_PCD_TX;
                        return ERR_BUSY;
                    }
                    return ERR_TIMEOUT; /* NFC Forum mandates timeout or
                                           transmission error depending on
                                           previous errors */
                }
        ...
        /*******************************************************************************/
            ...
            if (rfalIsoDep_PCBisChaining(rxPCB)) {
                ...
                *outIsChaining = true;
                ...
                if (rfalIsoDep_GetBN(rxPCB) == gIsoDep.blockNumber) {
                    /* Rule B - ACK with correct block number -> Increase Block
                     * number */
                    rfalIsoDep_ToggleBN(gIsoDep.blockNumber);

                    rfalIsoDepClearCounters(); /* Clear counters in case R
                                                  counter is already at max */
                    ...
                    /* remove ISO DEP header, check is necessary to move the INF
                     * data on the buffer */
                    *outActRxLen -= gIsoDep.hdrLen;
                    if ((gIsoDep.hdrLen != gIsoDep.rxBufInfPos) &&
                        (*outActRxLen > 0U)) {
                        ST_MEMMOVE(&gIsoDep.rxBuf[gIsoDep.rxBufInfPos],
                                   &gIsoDep.rxBuf[gIsoDep.hdrLen],
                                   *outActRxLen);
                    }

                    rfalIsoDepClearCounters();
                    return ERR_AGAIN; /* Send Again signalling to run again, but
            ...
            if (rfalIsoDep_GetBN(rxPCB) == gIsoDep.blockNumber) {
                /* Rule B - I-Block with correct block number -> Increase Block
                 * number */
                rfalIsoDep_ToggleBN(gIsoDep.blockNumber);
                ...
                *outActRxLen -= gIsoDep.hdrLen;
                ...
                if ((gIsoDep.compMode != RFAL_COMPLIANCE_MODE_ISO)) {
                    /* Invalid Block (not chaining) -> Raise error
                     * Digital 1.1  15.2.6.4   EMVCo 2.6  10.3.5.4 */
                    return ERR_PROTO;
                }
...
rfalIsoDepDeselect(void)
{
    ...
    /*******************************************************************************/
    /* The Deselect process is being done blocking, Digital 1.0 - 13.2.7.1 MUST
     * wait response and retry*/
    /* Set the maximum reruns while we will wait for a response */
    cntRerun = ISODEP_MAX_RERUNS;

    /* Send DSL request and run protocol until get a response, error or
     * "timeout" */
    EXIT_ON_ERR(ret,
                rfalIsoDepHandleControlMsg(ISODEP_S_DSL, RFAL_ISODEP_NO_PARAM));
    do {
...
rfalIsoDepFWI2FWT(uint8_t fwi)
{
    ...
    result = MIN(RFAL_ISODEP_MAX_FWT,
...
/*******************************************************************************/
uint16_t
rfalIsoDepFSxI2FSx(uint8_t FSxI)
{
    uint16_t fsx;
    uint8_t fsi;
    ...
    fsi = ((gIsoDep.compMode == RFAL_COMPLIANCE_MODE_EMV)
               ? MIN(FSxI, RFAL_ISODEP_FSDI_MAX_EMV)
...
/*******************************************************************************/
bool
rfalIsoDepIsRats(const uint8_t *buf, uint8_t bufLen)
{
    if (buf != NULL) {
...
/*******************************************************************************/
bool
rfalIsoDepIsAttrib(const uint8_t *buf, uint8_t bufLen)
{
    if (buf != NULL) {
...
/*******************************************************************************/
ReturnCode
rfalIsoDepListenStartActivation(rfalIsoDepAtsParam *atsParam,
                                ...
                                rfalIsoDepListenActvParam actParam)
{
    ...
    txBuf = (uint8_t *)
                actParam.rxBuf; /* Use the rxBuf as TxBuf as well, the struct
                                   enforces a size enough MAX( NFCA_ATS_MAX_LEN,
                                   NFCB_ATTRIB_RES_MAX_LEN ) */
    ...
    if (*buffer == RFAL_ISODEP_CMD_RATS) {
        ...
        if (actParam.isoDepDev != NULL) {
            ST_MEMCPY((uint8_t *)&actParam.isoDepDev->activation.A.Poller.RATS,
                      buffer,
        ...
        gIsoDep.fsx = rfalIsoDepFSxI2FSx(
            (((*buffer) & RFAL_ISODEP_RATS_PARAM_FSDI_MASK) >>
        ...
        txBuf[bufIt++] =
            (RFAL_ISODEP_ATS_HIST_OFFSET + atsParam->hbLen); /* TL */
        ...
        txBuf[bufIt++] =
            (uint8_t)((atsParam->didSupport) ? RFAL_ISODEP_ATS_TC_DID
                                             : 0U); /* TC */
...
/*******************************************************************************/
ReturnCode
rfalIsoDepListenGetActivationStatus(void)
{
    ReturnCode err;
    uint8_t *txBuf;
    uint8_t bufIt;
    ...
    txBuf =
        (uint8_t *)gIsoDep.actvParam
            .rxBuf;     /* Use the rxBuf as TxBuf as well, the struct enforces a
                           size enough  MAX(NFCA_PPS_RES_LEN, ISODEP_DSL_MAX_LEN) */
    dri = RFAL_BR_KEEP; /* The RFAL_BR_KEEP is used to check if PPS with BR
    ...
    if (gIsoDep.state == ISODEP_ST_PICC_ACT_ATS) {
        ...
        if ((((uint8_t *)
            ...
            if (((((uint8_t *)
                ...
                ((((uint8_t *)
                       gIsoDep.actvParam.rxBuf)[RFAL_ISODEP_PPS_PPS0_POS] &
                  RFAL_ISODEP_PPS0_VALID_MASK) !=
                 RFAL_ISODEP_PPS0_PPS1_NOT_PRESENT)) {
                /* Invalid DID on PPS request or Invalid PPS0, reEnable the
                 * receiver and wait another frame */
                rfalIsoDepReEnableRx((uint8_t *)gIsoDep.actvParam.rxBuf,
                                     sizeof(rfalIsoDepBufFormat),
                                     gIsoDep.actvParam.rxLen);
            ...
            if (((uint8_t *)
                ...
                uint8_t newdri =
                    ((uint8_t *)
                         gIsoDep.actvParam.rxBuf)[RFAL_ISODEP_PPS_PPS1_POS] &
                ...
                uint8_t newdsi =
                    ...
                    RFAL_ISODEP_PPS1_DxI_MASK; /* MISRA 10.8 */
                /* PRQA S 4342 2 # MISRA 10.5 - Layout of enum rfalBitRate and
                 * above masks guarantee no invalid enum values to be created */
            ...
            rfalTransceiveBlockingTx(
                txBuf, bufIt, (uint8_t *)gIsoDep.actvParam.rxBuf,
                sizeof(rfalIsoDepBufFormat), gIsoDep.actvParam.rxLen,
                RFAL_TXRX_FLAGS_DEFAULT, RFAL_FWT_NONE);
    ...
    gIsoDep.hdrLen = RFAL_ISODEP_PCB_LEN;
    gIsoDep.hdrLen +=
        RFAL_ISODEP_DID_LEN; /* Always assume DID to be aligned with Digital 1.1
                                15.1.2 and ISO14443  4 5.6.3    #454 */
    gIsoDep.hdrLen +=
        (uint8_t)((gIsoDep.nad != RFAL_ISODEP_NO_NAD) ? RFAL_ISODEP_NAD_LEN
                                                      : 0U);
    ...
    /* Activation done, keep the rcvd data in, reMap the activation buffer to
     * the global to be retrieved by the DEP method */
    gIsoDep.rxBuf = (uint8_t *)gIsoDep.actvParam.rxBuf;
    gIsoDep.rxBufLen = sizeof(rfalIsoDepBufFormat);
    gIsoDep.rxBufInfPos =
        (uint8_t)((uint32_t)gIsoDep.actvParam.rxBuf->inf -
...
/*******************************************************************************/
ReturnCode
rfalIsoDepStartTransceive(rfalIsoDepTxRxParam param)
{
    gIsoDep.txBuf = param.txBuf->prologue;
    gIsoDep.txBufInfPos =
        (uint8_t)((uint32_t)param.txBuf->inf - (uint32_t)param.txBuf->prologue);
    gIsoDep.txBufLen = param.txBufLen;
    gIsoDep.isTxChaining = param.isTxChaining;

    gIsoDep.rxBuf = param.rxBuf->prologue;
    gIsoDep.rxBufInfPos =
        (uint8_t)((uint32_t)param.rxBuf->inf - (uint32_t)param.rxBuf->prologue);
    gIsoDep.rxBufLen = sizeof(rfalIsoDepBufFormat);
    ...
    gIsoDep.ourFsx = ((param.ourFSx != RFAL_ISODEP_FSX_KEEP) ? param.ourFSx
                                                             : gIsoDep.ourFsx);
    ...
    if (gIsoDep.role == ISODEP_ROLE_PICC) {
        if (gIsoDep.txBufLen > 0U) {
            ...
            if (!gIsoDep.isWait4WTX) {
                ...
                /* If RTOX Ack is expected, signal a pending Tx to be
                 * transmitted right after */
                gIsoDep.isTxPending = true;
            }
        }

        /* Digital 1.1  15.2.5.1 The first block SHALL be sent by the
         * Reader/Writer */
        gIsoDep.state = ISODEP_ST_PICC_RX;
        return ERR_NONE;
    }
...
/*******************************************************************************/
ReturnCode
rfalIsoDepGetTransceiveStatus(void)
{
    if (gIsoDep.role == ISODEP_ROLE_PICC) {
#if RFAL_FEATURE_ISO_DEP_LISTEN
...
rfalIsoDepDataExchangePICC(void)
{
    ...
    case ISODEP_ST_PICC_RX:
        ...
        case ERR_FRAMING:
            ...
            /* Digital 1.1 - 15.2.6.2  The CE SHALL NOT attempt error recovery
             * and remains in Rx mode upon Transmission or a Protocol Error */
    ...
    case ISODEP_ST_PICC_SWTX:
        ...
        /* Digital 1.1  15.2.2.9 - Calculate the WTXM such that FWTtemp <=
         * FWTmax */
        gIsoDep.lastWTXM = (uint8_t)rfalIsoDep_WTXMListenerMax(gIsoDep.fwt);
        EXIT_ON_ERR(ret,
                    rfalIsoDepHandleControlMsg(ISODEP_S_WTX, gIsoDep.lastWTXM));

        gIsoDep.state =
            ISODEP_ST_PICC_RX; /* Go back to Rx to process WTX ack        */
        return ERR_BUSY;
    ...
    /*******************************************************************************/
    /* When DID=0 PCD may or may not use DID, therefore check whether current
     * PCD request has DID present to be reflected on max INF length #454  */
    ...
    /* Store whether last PCD block had DID. for PICC special handling of DID =
     * 0 */
    ...
    /* If we are expecting DID, check if PCB signals its presence and if device
     * ID match OR If our DID=0 and DID is sent but with an incorrect value */
    if (((gIsoDep.did != RFAL_ISODEP_DID_00) &&
         ((!rfalIsoDep_PCBhasDID(rxPCB)) ||
          (gIsoDep.did != gIsoDep.rxBuf[ISODEP_DID_POS]))) ||
    ...
    if (rfalIsoDep_PCBisSBlock(rxPCB)) {
        ...
        if (rfalIsoDep_PCBisSWTX(rxPCB)) {
            ...
            if (rfalIsoDep_PCBisWTX(gIsoDep.lastPCB)) {
                /* Digital 1.1  15.2.2.11 S(WTX) Ack with different WTXM ->
                 * Protocol Error  * Power level indication also should be set
                 * to 0              */
        ...
        if (rfalIsoDep_PCBisSDeselect(rxPCB) &&
            ...
            EXIT_ON_ERR(ret, rfalIsoDepHandleControlMsg(ISODEP_S_DSL,
    ...
    /*******************************************************************************/
        ...
        if (rfalIsoDep_PCBisRACK(rxPCB)) /* Check if is a R-ACK */
        {
                ...
                gIsoDep.blockNumber) /* Check block number  */
            {
                ...
                /* This block has been transmitted and acknowledged, perform WTX
                 * until next data is provided  */

                /* Rule 9 - PICC is allowed to send an S(WTX) instead of an
                 * I-block or an R(ACK) */
                rfalIsoDepTimerStart(
                    gIsoDep.WTXTimer,
    ...
    /*******************************************************************************/
        ...
        if ((rfalIsoDep_GetBN(rxPCB) != gIsoDep.blockNumber) ||
            ...
            /* ISO 14443-4 7.5.6.2 & Digital 1.1 - 15.2.6.2  The CE SHALL NOT
             * attempt error recovery and remains in Rx mode upon Transmission
             * or a Protocol Error */
            rfalIsoDepReEnableRx((uint8_t *)gIsoDep.rxBuf,
                                 sizeof(rfalIsoDepBufFormat), gIsoDep.rxLen);
            return ERR_BUSY;
        ...
        if (rfalIsoDep_PCBisChaining(rxPCB)) {
            ...
            *gIsoDep.rxChaining = true; /* Output Parameter*/
            ...
            EXIT_ON_ERR(ret, rfalIsoDepHandleControlMsg(ISODEP_R_ACK,
                                                        RFAL_ISODEP_NO_PARAM));
            ...
            *gIsoDep.rxLen -= gIsoDep.hdrLen;
            ...
            return ERR_AGAIN; /* Send Again signalling to run again, but some
...
rfalIsoDepGetRATSStatus(void)
{
    ...
    if (ret == ERR_NONE) {
        ...
        /* Check and assign if ATS length was requested (length also available
         * on TL) */
...
/*******************************************************************************/
ReturnCode
rfalIsoDepRATS(rfalIsoDepFSxI FSDI, uint8_t DID, rfalIsoDepAts *ats,
               uint8_t *atsLen)
{
    ReturnCode ret;
...
/*******************************************************************************/
ReturnCode
rfalIsoDepPPS(uint8_t DID, rfalBitRate DSI, rfalBitRate DRI,
              rfalIsoDepPpsRes *ppsRes)
{
    ReturnCode ret;
                      ...
                      uint8_t *attribResLen)
{
    ...
    if ((HLInfo != NULL) && (HLInfoLen > 0U)) {
        ST_MEMCPY(gIsoDep.actv.attribReq.HLInfo, HLInfo,
                  MIN(HLInfoLen, RFAL_ISODEP_ATTRIB_HLINFO_LEN));
    }
    ...
    rfalCreateByteFlagsTxRxContext(
        ...
        &gIsoDep.rxBufLen, RFAL_TXRX_FLAGS_DEFAULT, fwt);
    return rfalStartTransceive(&ctx);
}
...
/*******************************************************************************/
ReturnCode
rfalIsoDepATTRIB(const uint8_t *nfcid0, uint8_t PARAM1, rfalBitRate DSI,
                 rfalBitRate DRI, rfalIsoDepFSxI FSDI, uint8_t PARAM3,
                 uint8_t DID, const uint8_t *HLInfo, uint8_t HLInfoLen,
                 uint32_t fwt, rfalIsoDepAttribRes *attribRes,
                 uint8_t *attribResLen)
{
    ...
    EXIT_ON_ERR(ret, rfalIsoDepStartATTRIB(nfcid0, PARAM1, DSI, DRI, FSDI,
                                           PARAM3, DID, HLInfo, HLInfoLen, fwt,
                                           attribRes, attribResLen));
...
/*******************************************************************************/
ReturnCode
rfalIsoDepPollAHandleActivation(rfalIsoDepFSxI FSDI, uint8_t DID,
                                rfalBitRate maxBR,
...
/*******************************************************************************/
ReturnCode
rfalIsoDepPollAStartActivation(rfalIsoDepFSxI FSDI, uint8_t DID,
                               rfalBitRate maxBR,
                               rfalIsoDepDevice *rfalIsoDepDev)
{
    ReturnCode ret;
    ...
    /* Enable EMD supresssion|handling according to  Digital 2.1  4.1.1.1 ;
     * EMVCo 3.0  4.9.2 ; ISO 14443-3  8.3 */
    rfalSetErrorHandling(RFAL_ERRORHANDLING_EMD);

    /* Start RATS Transceive */
    EXIT_ON_ERR(ret, rfalIsoDepStartRATS(
                         FSDI, DID, &rfalIsoDepDev->activation.A.Listener.ATS,
...
/*******************************************************************************/
ReturnCode
rfalIsoDepPollAGetActivationStatus(void)
{
    ReturnCode ret;
    uint8_t msgIt;
    rfalBitRate maxBR;
        ...
        if (ret != ERR_BUSY) {
            if (ret != ERR_NONE) {
                ...
                if (gIsoDep.cntRRetrys != 0U) {
                    ...
                    /* Send RATS retransmission */ /* PRQA S 4342 1 # MISRA 10.5
                                                      - Layout of enum
                                                      rfalIsoDepFSxI is
                                                      guaranteed whithin 4bit
                                                      range */
                    EXIT_ON_ERR(
                        ret,
                        rfalIsoDepStartRATS(
                            (rfalIsoDepFSxI)(uint8_t)(gIsoDep.actv.ratsReq
                                                          .PARAM >>
                                                      RFAL_ISODEP_RATS_PARAM_FSDI_SHIFT),
                            gIsoDep.did,
                 ...
                 *   ISO14443-4  5.6.1  If RATS fails, a Deactivation sequence
                 * should be performed as defined on clause 8 Activity 1.1  9.6
                 * Device Deactivation Activity is to be only performed when
                 * there's an active device */
            ...
            } else /* ATS received */
            {
                maxBR = gIsoDep.actvDev->info
                            .DSI; /* Retrieve requested max bitrate */
                ...
                gIsoDep.actvDev->info.FSxI =
                    (uint8_t)RFAL_ISODEP_FSXI_32; /* FSC default value is 32
                                                     bytes  ISO14443-A  5.2.3 */
                ...
                if (gIsoDep.actvDev->activation.A.Listener.ATS.TL >
                    ...
                    if ((gIsoDep.actvDev->activation.A.Listener.ATS.T0 &
                        ...
                        /* Advanced protocol features defined on Digital 1.0
                         * Table 69, removed after */
                ...
                if ((gIsoDep.actvDev->info.DSI != RFAL_BR_106) ||
                    (gIsoDep.actvDev->info.DRI != RFAL_BR_106)) {
                    /* Send PPS */ /*  PRQA S 0310 1 # MISRA 11.3 - Intentional
                                      safe cast to avoiding buffer duplication
                                    */
                    EXIT_ON_ERR(ret, rfalIsoDepStartPPS(
                                         gIsoDep.actvDev->info.DID,
                                         gIsoDep.actvDev->info.DSI,
                                         gIsoDep.actvDev->info.DRI,
...
/*******************************************************************************/
ReturnCode
rfalIsoDepPollBHandleActivation(rfalIsoDepFSxI FSDI, uint8_t DID,
                                ...
                                rfalIsoDepDevice *rfalIsoDepDev)
{
    ...
    EXIT_ON_ERR(ret, rfalIsoDepPollBStartActivation(FSDI, DID, maxBR, PARAM1,
                                                    nfcbDev, HLInfo, HLInfoLen,
                                                    rfalIsoDepDev));
...
/*******************************************************************************/
ReturnCode
rfalIsoDepPollBStartActivation(rfalIsoDepFSxI FSDI, uint8_t DID,
                               rfalBitRate maxBR, uint8_t PARAM1,
                               const rfalNfcbListenDevice *nfcbDev,
                               const uint8_t *HLInfo, uint8_t HLInfoLen,
                               rfalIsoDepDevice *rfalIsoDepDev)
{
    ...
    rfalIsoDepDev->info.FWI = ((nfcbDev->sensbRes.protInfo.FwiAdcFo >>
                                RFAL_NFCB_SENSB_RES_FWI_SHIFT) &
    ...
    if (gIsoDep.compMode == RFAL_COMPLIANCE_MODE_EMV) {
        /* Disregard Minimum TR2 returned by PICC, always use FDTb MIN EMVCo 3.0
         * 6.3.2.10  */
        rfalSetFDTPoll(RFAL_FDT_POLL_NFCB_POLLER);
    } else {
        /* Apply minimum TR2 from SENSB_RES   Digital 2.1  7.6.2.23 */
        rfalSetFDTPoll(
            rfalNfcbTR2ToFDT(((nfcbDev->sensbRes.protInfo.FsciProType >>
...
/*******************************************************************************/
ReturnCode
rfalIsoDepPollBGetActivationStatus(void)
{
    ReturnCode ret;
    uint8_t mbli;
    ...
    if (ret != ERR_BUSY) {
        if (ret == ERR_NONE) {
            ...
            if (mbli > 0U) {
                /* Digital 1.1  14.6.2  Calculate Maximum Buffer Length MBL =
                 * FSC x 2^(MBLI-1) */
...
/*******************************************************************************/
ReturnCode
rfalIsoDepPollHandleSParameters(rfalIsoDepDevice *rfalIsoDepDev,
                                rfalBitRate maxTxBR, rfalBitRate maxRxBR)
{
    uint8_t it;
    uint8_t supPCD2PICC;
    uint8_t supPICC2PCD;
    ...
    EXIT_ON_ERR(ret,
                rfalTransceiveBlockingTxRx(
                    ...
                    (uint8_t *)&sParam, sizeof(rfalIsoDepControlMsgSParam),
                    &rcvLen, RFAL_TXRX_FLAGS_DEFAULT, ISODEP_FWT_DEACTIVATION));
    ...
    for (it = 0; it <= (uint8_t)maxTxBR; it++) {
        if ((supPCD2PICC & (0x01U << it)) != 0U) {
            txBR = (rfalBitRate)
                it; /* PRQA S 4342 # MISRA 10.5 - Layout of enum rfalBitRate and
                       above clamping of maxTxBR guarantee no invalid enum
    ...
    EXIT_ON_ERR(ret, rfalTransceiveBlockingTxRx(
                         (uint8_t *)&sParam,
                         (RFAL_ISODEP_SPARAM_HDR_LEN + (uint16_t)it),
                         (uint8_t *)&sParam, sizeof(rfalIsoDepControlMsgSParam),
                      ...
                      rfalBitRate *dsi, rfalBitRate *dri)
{
    ...
    do {
        ...
        /* Digital 1.0  5.6.2.5 & 11.6.2.14: A received RFU value of b4 = 1b
         * MUST be interpreted as if b7 to b1 ? 0000000b (only 106 kbits/s in
         * both direction) */
        ...
        for (i = 2; i >= 0; i--) /* Check supported bit rate from the highest */
        {
            if (((dsiMask & (0x10U << (uint8_t)i)) != 0U) &&
                ...
                (*dsi) = (rfalBitRate)
                    newdsi; /* PRQA S 4342 # MISRA 10.5 - Layout of enum
                               rfalBitRate and range of loop variable guarantee
                               no invalid enum values to be created */
                break;
            }
        }
        ...
        for (i = 2; i >= 0; i--) /* Check supported bit rate from the highest */
        {
            if (((driMask & (0x01U << (uint8_t)i)) != 0U) &&
                ...
                (*dri) = (rfalBitRate)
                    newdri; /* PRQA S 4342 # MISRA 10.5 - Layout of enum
                               rfalBitRate and range of loop variable guarantee
                               no invalid enum values to be created */
                break;
            }
        ...
        /* Digital 1.0 Table 67: if b8=1b, then only the same bit rate divisor
         * for both directions is supported */
        if ((piccBRCapability & RFAL_ISODEP_SAME_BITRATE_MASK) != 0U) {
            ...
            if ((RFAL_BR_106 != (*dsi)) &&
                ...
                bitrateFound = false;
                curMaxBR = (*dsi); /* set allowed bitrate to be lowest and
                                      determine bit rate again */
            }
        }
    } while (!(bitrateFound));
}
                           ...
                           uint16_t rxPos)
{
    ...
    iBlockParam->txBuf =
        (rfalIsoDepBufFormat *)
            apduParam.txBuf; /*  PRQA S 0310 # MISRA 11.3 - Intentional safe
                                cast to avoiding large buffer duplication */
    iBlockParam->rxBuf =
        apduParam.tmpBuf; /* Simply using the apdu buffer is not possible
                             because of current ACK handling */
...
/*******************************************************************************/
ReturnCode
rfalIsoDepStartApduTransceive(rfalIsoDepApduTxRxParam param)
{
...
/*******************************************************************************/
ReturnCode
rfalIsoDepGetApduTransceiveStatus(void)
{
    ReturnCode ret;
    rfalIsoDepTxRxParam txRxParam;
    ...
    case ERR_NONE:
        ...
        if (gIsoDep.isTxChaining) {
            ...
            if (txRxParam.txBufLen > 0U) /* MISRA 21.18 */
            {
                ...
                ST_MEMCPY(gIsoDep.APDUParam.txBuf->apdu,
    ...
    case ERR_AGAIN: /*  PRQA S 2003 # MISRA 16.3 - Intentional fall through */
        ...
        if (gIsoDep.APDUParam.rxLen == NULL) {
            if (ret == ERR_AGAIN) {
                /* In Listen mode first chained packet cannot be retrieved via
                 * APDU interface */
                return ERR_NOTSUPP;
            }
        ...
        if (*gIsoDep.APDUParam.rxLen > 0U) /* MISRA 21.18 */
        {
            ...
            ST_MEMCPY(&gIsoDep.APDUParam.rxBuf->apdu[gIsoDep.APDURxPos],
