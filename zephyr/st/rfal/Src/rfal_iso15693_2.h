 ...
 *  \param[in,out] offset : Set to 0 for first transfer, function will update it
 to point to next byte to be coded
 *  \param[out] outbuf : buffer where the function will store the coded subbit
 stream
 ...
 *
 *  \return ERR_IO : Error during communication.
 *  \return ERR_AGAIN : Data was not coded all the way. Call function again with
 a new/emptied buffer
 *  \return ERR_NO_MEM : In case outBuf is not big enough. Needs to have at
                         least 5 bytes for 1of4 coding and 65 bytes for 1of256
 coding
 *  \return ERR_NONE : No error.
 *
 *****************************************************************************
 ...
 *  \param[in] outBufLen : Length of output buffer, should be approx twice the
 *size of inBuf \param[out] outBufPos : The number of decoded bytes. Could be
 *used in extended implementation to allow multiple calls \param[out]
 *bitsBeforeCol : in case of ERR_RF_COLLISION this value holds the number of
 *bits in the current byte where the collision happened. \param[in] ignoreBits :
