...
#define RFAL_FEATURE_NFCF                                                      \
    false /* NFC-F module configuration missing. Disabled by default */
#endif
...
#define rfalNfcfSlots2CardNum(s)                                               \
    ((uint8_t)(s) +                                                            \
     1U) /*!< Converts Time Slot Number (TSN) into num of slots  */
...
typedef struct {
    ...
    rfalFeliCaPollRes
        POLL_F[RFAL_NFCF_POLL_MAXCARDS]; /*!< GRE_POLL_F   Activity 1.0 Table 10
                                          */
} rfalNfcfGreedyF;
...
*/
static void
rfalNfcfComputeValidSENF(rfalNfcfListenDevice *outDevInfo, uint8_t *curDevIdx,
                         uint8_t devLimit, bool overwrite, bool *nfcDepFound);
                         ...
                         uint8_t devLimit, bool overwrite, bool *nfcDepFound)
{
    ...
    while ((gNfcf.CR.greedyF.pollFound > 0U) && ((*curDevIdx) < devLimit)) {
        ...
        /* MISRA 11.3 - Cannot point directly into different object type, use
         * local copy */
        ST_MEMCPY(
            (uint8_t *)&sensfCopy,
        ...
        for (tmpIdx = 0; tmpIdx < (*curDevIdx); tmpIdx++) {
            if (ST_BYTECMP(sensfBuf->SENSF_RES.NFCID2,
        ...
        if (duplicate && overwrite) {
            ...
            ST_MEMCPY(&outDevInfo[tmpIdx].sensfRes, &sensfBuf->SENSF_RES,
                      outDevInfo[tmpIdx].sensfResLen);
            continue;
        } else {
            ...
            outDevInfo[(*curDevIdx)].sensfResLen =
                (sensfBuf->LEN - RFAL_NFCF_LENGTH_LEN);
            ST_MEMCPY(&outDevInfo[(*curDevIdx)].sensfRes, &sensfBuf->SENSF_RES,
                      outDevInfo[(*curDevIdx)].sensfResLen);
        }
...
/*******************************************************************************/
ReturnCode
rfalNfcfPollerInitialize(rfalBitRate bitRate)
{
    ReturnCode ret;
...
/*******************************************************************************/
ReturnCode
rfalNfcfPollerPoll(rfalFeliCaPollSlots slots, uint16_t sysCode, uint8_t reqCode,
                   rfalFeliCaPollRes *cardList, uint8_t *devCnt,
                   uint8_t *collisions)
{
...
/*******************************************************************************/
ReturnCode
rfalNfcfPollerCheckPresence(void)
{
    ReturnCode ret;
...
/*******************************************************************************/
ReturnCode
rfalNfcfPollerStartCheckPresence(void)
{
    gNfcf.CR.greedyF.pollFound = 0;
    gNfcf.CR.greedyF.pollCollision = 0;

    /* ACTIVITY 1.0 & 1.1 - 9.2.3.17 SENSF_REQ  must be with number of slots
     * equal to 4 SC must be 0xFFFF RC must be 0x00 (No system code info
     * required) */
    return rfalStartFeliCaPoll(
        RFAL_FELICA_4_SLOTS, RFAL_NFCF_SYSTEMCODE,
        RFAL_FELICA_POLL_RC_NO_REQUEST, gNfcf.CR.greedyF.POLL_F,
        rfalNfcfSlots2CardNum(RFAL_FELICA_4_SLOTS), &gNfcf.CR.greedyF.pollFound,
        &gNfcf.CR.greedyF.pollCollision);
}

/*******************************************************************************/
ReturnCode
rfalNfcfPollerGetCheckPresenceStatus(void)
{
...
/*******************************************************************************/
ReturnCode
rfalNfcfPollerCollisionResolution(rfalComplianceMode compMode, uint8_t devLimit,
                                  rfalNfcfListenDevice *nfcfDevList,
                                  uint8_t *devCnt)
{
    ReturnCode ret;

    EXIT_ON_ERR(ret, rfalNfcfPollerStartCollisionResolution(
...
/*******************************************************************************/
ReturnCode
rfalNfcfPollerStartCollisionResolution(rfalComplianceMode compMode,
                                       uint8_t devLimit,
                                       rfalNfcfListenDevice *nfcfDevList,
                                       uint8_t *devCnt)
{
    ...
    *devCnt = 0;
    ...
    /*******************************************************************************************/
    rfalNfcfComputeValidSENF(nfcfDevList, devCnt,
                             ((devLimit == 0U)
...
/*******************************************************************************/
ReturnCode
rfalNfcfPollerGetCollisionResolutionStatus(void)
{
    ReturnCode ret;
    ...
    case RFAL_NFCF_CR_POLL_SC:
        ...
        if (gNfcf.CR.state == RFAL_NFCF_CR_POLL) {
            /*******************************************************************************/
            /* Activity 2.1  9.3.6.3  - Symbol 2 Check if devices found are
            ...
            /*******************************************************************************/
            /* Activity 1.0 - 9.3.6.5  Copy valid SENSF_RES and then to remove
             * it          */
            /* Activity 1.1 - 9.3.6.65 Copy and filter duplicates */
            /* For now, due to some devices keep generating different nfcid2, we
             * use 1.0   */
            /* Phones detected: Samsung Galaxy Nexus,Samsung Galaxy S3,Samsung
             * Nexus S     */
            /*******************************************************************************/
            *gNfcf.CR.devCnt = 0;
        }
    ...
    case RFAL_NFCF_CR_PARSE:
        ...
        /*******************************************************************************/
        /* Activity 2.1  9.3.6.6 - Symbol 5  Check if any device supports NFC
         * DEP       */
        if ((gNfcf.CR.nfcDepFound) &&
            ...
            /* Set compliance mode to invalid (non NFC) to poll for NFC-DEP
             * devices only once */
...
/*******************************************************************************/
ReturnCode
rfalNfcfPollerCheck(const uint8_t *nfcid2,
                    const rfalNfcfServBlockListParam *servBlock, uint8_t *rxBuf,
                    uint16_t rxBufLen, uint16_t *rcvdLen)
{
    ...
    for (i = 0; i < servBlock->numServ; i++) {
        txBuf[msgIt++] = (uint8_t)((servBlock->servList[i] >> 0U) &
                                   0xFFU); /* Service Code    */
    ...
    for (i = 0; i < servBlock->numBlock; i++) {
            ...
            0U) /* Check if 2 or 3 byte block list element       */
        {
            ...
            txBuf[msgIt++] =
                (uint8_t)((servBlock->blockList[i].blockNum >> 0U) &
    ...
    if (ret == ERR_NONE) {
        ...
        /* CHECK succesfull, remove header */
            ...
            (*rcvdLen) -=
                (RFAL_NFCF_LENGTH_LEN + RFAL_NFCF_CHECKUPDATE_RES_NOB_POS);

            if (*rcvdLen > 0U) {
                ST_MEMMOVE(rxBuf, &checkRes[RFAL_NFCF_CHECKUPDATE_RES_NOB_POS],
                           (*rcvdLen));
            }
        }
    }
...
/*******************************************************************************/
ReturnCode
rfalNfcfPollerUpdate(const uint8_t *nfcid2,
...
/*******************************************************************************/
bool
rfalNfcfListenerIsT3TReq(const uint8_t *buf, uint16_t bufLen, uint8_t *nfcid2)
{
    /* Check cmd byte */
    switch (*buf) {
