...
#define RFAL_FEATURE_T2T                                                       \
    false /* T2T module configuration missing. Disabled by default */
#endif
...
#define RFAL_FDT_POLL_READ_MAX                                                 \
    rfalConvMsTo1fc(5U) /*!< Maximum Wait time for Read command as defined in  \
                           TS T2T 1.0 table 18   */
#define RFAL_FDT_POLL_WRITE_MAX                                                \
    rfalConvMsTo1fc(10U) /*!< Maximum Wait time for Write command as defined   \
                            in TS T2T 1.0 table 18  */
...
ReturnCode
rfalT2TPollerRead(uint8_t blockNum, uint8_t *rxBuf, uint16_t rxBufLen,
                  uint16_t *rcvLen)
{
    ReturnCode ret;
    rfalT2TReadReq req;
    ...
    /* T2T 1.0 5.2.1.7 The Reader/Writer SHALL treat a NACK in response to a
     * READ Command as a Protocol Error */
...
/*******************************************************************************/
ReturnCode
rfalT2TPollerWrite(uint8_t blockNum, const uint8_t *wrData)
{
    ReturnCode ret;
    rfalT2TWriteReq req;
    uint8_t res;
...
/*******************************************************************************/
ReturnCode
rfalT2TPollerSectorSelect(uint8_t sectorNum)
{
    ...
    ret = rfalTransceiveBlockingTxRx(
        (uint8_t *)&p1Req, sizeof(rfalT2TSectorSelectP1Req), &res,
        sizeof(uint8_t), &rxLen, RFAL_TXRX_FLAGS_DEFAULT, RFAL_FDT_POLL_SL_MAX);
    ...
    ret = rfalTransceiveBlockingTxRx(
        (uint8_t *)&p2Req, sizeof(rfalT2TSectorSelectP2Req), &res,
        sizeof(uint8_t), &rxLen, RFAL_TXRX_FLAGS_DEFAULT, RFAL_FDT_POLL_SL_MAX);

    /* T2T 1.0 5.4.1.14 The Reader/Writer SHALL treat any response received
     * before the end of PATT2T,SL,MAX as a Protocol Error */
