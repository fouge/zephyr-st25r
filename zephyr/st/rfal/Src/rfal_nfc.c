...
*/
static ReturnCode
rfalNfcPollTechDetetection(void);
static ReturnCode
rfalNfcPollCollResolution(void);
static ReturnCode
rfalNfcPollActivation(uint8_t devIt);
static ReturnCode
rfalNfcDeactivation(void);

#if RFAL_FEATURE_NFC_DEP
static ReturnCode
rfalNfcNfcDepActivate(rfalNfcDevice *device, rfalNfcDepCommMode commMode,
                      const uint8_t *atrReq, uint16_t atrReqLen);
#endif /* RFAL_FEATURE_NFC_DEP */

#if RFAL_FEATURE_LISTEN_MODE
static ReturnCode
rfalNfcListenActivation(void);
#endif /* RFAL_FEATURE_LISTEN_MODE*/

/*******************************************************************************/
ReturnCode
rfalNfcInitialize(void)
{
    ReturnCode err;
...
/*******************************************************************************/
ReturnCode
rfalNfcDiscover(const rfalNfcDiscoverParam *disParams)
{
    ...
    if ((((disParams->techs2Find & RFAL_NFC_POLL_TECH_A) != 0U) &&
        ...
        (((disParams->techs2Find & RFAL_NFC_LISTEN_TECH_AP2P) != 0U) &&
         (!((bool)RFAL_FEATURE_NFC_DEP)))) {
        return ERR_DISABLED; /*  PRQA S  2880 # MISRA 2.1 - Unreachable code due
    ...
    gNfcDev.lmMask = 0U;
    gNfcDev.lmMask |=
        (((gNfcDev.disc.techs2Find & RFAL_NFC_LISTEN_TECH_A) != 0U)
             ? RFAL_LM_MASK_NFCA
             : 0U);
    gNfcDev.lmMask |=
        (((gNfcDev.disc.techs2Find & RFAL_NFC_LISTEN_TECH_B) != 0U)
             ? RFAL_LM_MASK_NFCB
             : 0U);
    gNfcDev.lmMask |=
        (((gNfcDev.disc.techs2Find & RFAL_NFC_LISTEN_TECH_F) != 0U)
             ? RFAL_LM_MASK_NFCF
             : 0U);
    gNfcDev.lmMask |=
...
/*******************************************************************************/
ReturnCode
rfalNfcDeactivate(rfalNfcDeactivateType deactType)
{
...
/*******************************************************************************/
ReturnCode
rfalNfcGetDevicesFound(rfalNfcDevice **devList, uint8_t *devCnt)
{
    /* Check for valid state */
    if (gNfcDev.state < RFAL_NFC_STATE_POLL_SELECT) {
        return ERR_WRONG_STATE;
...
/*******************************************************************************/
ReturnCode
rfalNfcGetActiveDevice(rfalNfcDevice **dev)
{
    /* Check for valid state */
...
rfalNfcWorker(void)
{
    ...
    case RFAL_NFC_STATE_START_DISCOVERY:
        ...
        if ((gNfcDev.disc.wakeupEnabled) &&
            ...
            if (err == ERR_NONE) {
                ...
                rfalNfcNfcNotify(
                    gNfcDev.state); /* Notify caller that WU was started */
            }
        }
        gNfcDev.techDctCnt++;
    ...
    case RFAL_NFC_STATE_WAKEUP_MODE:
        ...
        if (rfalWakeUpModeHasWoke()) {
            ...
            rfalNfcNfcNotify(
                gNfcDev.state); /* Notify caller that WU has woke */
        }
#endif /* RFAL_FEATURE_WAKEUP_MODE */
    ...
    case RFAL_NFC_STATE_POLL_TECHDETECT:
        ...
        if (err != ERR_BUSY) /* Wait until all technologies are performed */
        {
                                         ...
                                         were found   */
            {
                rfalFieldOff();
                gNfcDev.state =
                    RFAL_NFC_STATE_LISTEN_TECHDETECT; /* Nothing found as
                                                         poller, go to listener
                                                       */
                break;
            }

            gNfcDev.techs2do =
                gNfcDev.techsFound; /* Store the found technologies for
                                       collision resolution */
            gNfcDev.state =
                RFAL_NFC_STATE_POLL_COLAVOIDANCE; /* One or more devices found,
    ...
    case RFAL_NFC_STATE_POLL_COLAVOIDANCE:
        ...
        if (err != ERR_BUSY) /* Wait until all technologies are performed */
        {
            ...
            if (gNfcDev.devCnt > 1U) {
                /* If more than one device was found inform upper layer to
                 * choose which one to activate */
                if (gNfcDev.disc.notifyCb != NULL) {
            ...
            /* If only one device or no callback has been set, activate the
             * first device found */
    ...
    case RFAL_NFC_STATE_POLL_ACTIVATION:
        ...
        if (err != ERR_BUSY) /* Wait until all Activation is complete */
        {
            if (err != ERR_NONE) /* Check if activation has failed        */
            {
                ...
                gNfcDev.deactType =
                    RFAL_NFC_DEACTIVATE_DISCOVERY; /* Ensure deactivation, not
                                                      Sleep        */
                gNfcDev.state =
                    RFAL_NFC_STATE_DEACTIVATION; /* If Activation failed,
                                                    restart loop    */
                break;
            }

            gNfcDev.state = RFAL_NFC_STATE_ACTIVATED; /* Device has been
                                                         properly activated */
    ...
    case RFAL_NFC_STATE_DATAEXCHANGE:
        ...
        if (err != ERR_WRONG_STATE) {
            if (gNfcDev.dataExErr !=
                ERR_BUSY) /* If Dataexchange has terminated */
            {
                gNfcDev.state =
    ...
    case RFAL_NFC_STATE_LISTEN_TECHDETECT:
        ...
        if (gNfcDev.lmMask !=
            0U) /* Check if configured to perform Listen mode */
        {
            err = rfalListenStart(
            ...
            if (err == ERR_NONE) {
                gNfcDev.state =
                    RFAL_NFC_STATE_LISTEN_COLAVOIDANCE; /* Wait for listen mode
                                                           to be activated */
            }
        }
        break;
    ...
    case RFAL_NFC_STATE_LISTEN_COLAVOIDANCE:
        ...
        if (platformTimerIsExpired(
                gNfcDev.discTmr)) /* Check if the total duration has been
                                     reached */
        {
            rfalListenStop();
            gNfcDev.state =
    ...
    case RFAL_NFC_STATE_LISTEN_SLEEP:
        ...
        if (err != ERR_BUSY) {
            if (err == ERR_NONE) {
                gNfcDev.activeDev = gNfcDev.devList; /* Assign the active device
                                                        to be used further on */
                gNfcDev.devCnt++;

                gNfcDev.state =
                    RFAL_NFC_STATE_ACTIVATED;    /* Device has been properly
                                                    activated */
                rfalNfcNfcNotify(gNfcDev.state); /* Inform upper layer that a
                                                    device has been activated */
            } else if ((!platformTimerIsExpired(gNfcDev.discTmr)) &&
                       (err == ERR_LINK_LOSS) &&
                       (gNfcDev.state == RFAL_NFC_STATE_LISTEN_ACTIVATION)) {
                break; /* Field|Link broken during activation, keep in Listen
                          the remaining total duration */
            } else {
                rfalListenStop();
                gNfcDev.state =
                    RFAL_NFC_STATE_START_DISCOVERY; /* Restart the discovery
                                                       loop */
                rfalNfcNfcNotify(gNfcDev.state); /* Notify caller             */
            }
        }
...
/*******************************************************************************/
ReturnCode
rfalNfcDataExchangeStart(uint8_t *txData, uint16_t txDataLen, uint8_t **rxData,
                         uint16_t **rvdLen, uint32_t fwt)
{
    ReturnCode err;
    rfalTransceiveContext ctx;

    /*******************************************************************************/
    /* The Data Exchange is divided in two different moments, the trigger/Start
     * of * the transfer followed by the check until its completion */
    if ((gNfcDev.state >= RFAL_NFC_STATE_ACTIVATED) &&
        (gNfcDev.activeDev != NULL)) {
        ...
        /*******************************************************************************/
        switch (
            gNfcDev.activeDev->rfInterface) /* Check which RF interface shall be
        ...
        case RFAL_NFC_INTERFACE_ISODEP: {
            ...
            if (txDataLen > 0U) {
                ST_MEMCPY((uint8_t *)gNfcDev.txBuf.isoDepBuf.apdu, txData,
                          txDataLen);
            }
        ...
        case RFAL_NFC_INTERFACE_NFCDEP: {
            ...
            if (txDataLen > 0U) {
                ST_MEMCPY((uint8_t *)gNfcDev.txBuf.nfcDepBuf.pdu, txData,
                          txDataLen);
            }
            ...
            rfalNfcDepTxRx.FSx =
                rfalNfcIsRemDevListener(gNfcDev.activeDev->type)
                    ? rfalNfcDepLR2FS((uint8_t)rfalNfcDepPP2LR(
                          gNfcDev.activeDev->proto.nfcDep.activation.Target
                              .ATR_RES.PPt))
                    : rfalNfcDepLR2FS((uint8_t)rfalNfcDepPP2LR(
                          gNfcDev.activeDev->proto.nfcDep.activation.Initiator
                              .ATR_REQ.PPi));
...
/*******************************************************************************/
ReturnCode
rfalNfcDataExchangeGetStatus(void)
{
    /*******************************************************************************/
    ...
    if (gNfcDev.state == RFAL_NFC_STATE_ACTIVATED) {
        ...
        if ((gNfcDev.activeDev->type == RFAL_NFC_POLL_TYPE_NFCF) &&
             ...
             * immediately                  */
            /* Can only call rfalGetTransceiveStatus() after starting a
             * transceive with rfalStartTransceive */
...
rfalNfcPollTechDetetection(void)
{
    ...
    if (((gNfcDev.disc.techs2Find & RFAL_NFC_POLL_TECH_AP2P) != 0U) &&
        ...
        if (!gNfcDev.isTechInit) {
            EXIT_ON_ERR(err,
                        rfalSetMode(RFAL_MODE_POLL_ACTIVE_P2P,
                                    gNfcDev.disc.ap2pBR, gNfcDev.disc.ap2pBR));
            ...
            EXIT_ON_ERR(err, rfalFieldOnAndStartGT()); /* Turns the Field On and
                                                          starts GT timer */
        ...
        if (rfalIsGTExpired()) /* Wait until Guard Time is fulfilled */
        {
            ...
            err =
                rfalNfcNfcDepActivate(gNfcDev.devList, RFAL_NFCDEP_COMM_ACTIVE,
                                      NULL, 0); /* Poll for NFC-A devices */
            if (err == ERR_NONE) {
    ...
    if (((gNfcDev.disc.techs2Find & RFAL_NFC_POLL_TECH_A) != 0U) &&
        ...
        if (!gNfcDev.isTechInit) {
            ...
            EXIT_ON_ERR(err,
                        rfalFieldOnAndStartGT()); /* As field is already On only
                                                     starts GT timer */
            gNfcDev.isTechInit = true;
    ...
    if (((gNfcDev.disc.techs2Find & RFAL_NFC_POLL_TECH_F) != 0U) &&
        ...
        if (rfalIsGTExpired()) /* Wait until Guard Time is fulfilled */
        {
            ...
            err = rfalNfcfPollerGetCheckPresenceStatus(); /* Poll for NFC-F
                                                             devices */
            if (err != ERR_BUSY) {
                if (err == ERR_NONE) {
    ...
    if (((gNfcDev.disc.techs2Find & RFAL_NFC_POLL_TECH_V) != 0U) &&
        ...
        if (rfalIsGTExpired()) /* Wait until Guard Time is fulfilled */
        {
            err = rfalNfcvPollerCheckPresence(
                &invRes); /* Poll for NFC-V devices */
            if (err == ERR_NONE) {
    ...
    if (((gNfcDev.disc.techs2Find & RFAL_NFC_POLL_TECH_ST25TB) != 0U) &&
        ...
        if (!gNfcDev.isTechInit) {
            EXIT_ON_ERR(
                err,
                rfalSt25tbPollerInitialize()); /* Initialize RFAL for NFC-V */
            EXIT_ON_ERR(err,
                        rfalFieldOnAndStartGT()); /* As field is already On only
                                                     starts GT timer */
            gNfcDev.isTechInit = true;
...
rfalNfcPollCollResolution(void)
{
                 ...
                 Resolution */
    {
        ...
        if (!gNfcDev.isOperOngoing) {
            EXIT_ON_ERR(err, rfalNfcaPollerStartFullCollisionResolution(
                                 gNfcDev.disc.compMode,
        ...
        if (err != ERR_BUSY) {
            ...
            if ((err == ERR_NONE) && (devCnt != 0U)) {
                for (i = 0; i < devCnt;
                     i++) /* Copy devices found form local Nfca list into global
                             device list */
                {
                    gNfcDev.devList[gNfcDev.devCnt].type =
                 ...
                 Resolution */
    {
        ...
        if (!gNfcDev.isTechInit) {
            EXIT_ON_ERR(
                ...
                rfalNfcbPollerInitialize()); /* Initialize RFAL for NFC-B */
            EXIT_ON_ERR(err,
                        rfalFieldOnAndStartGT()); /* Ensure GT again as other
        ...
        if (!gNfcDev.isOperOngoing) {
            EXIT_ON_ERR(err, rfalNfcbPollerStartCollisionResolution(
                                 gNfcDev.disc.compMode,
                                 (gNfcDev.disc.devLimit - gNfcDev.devCnt),
                                 nfcbDevList, &devCnt));
        ...
        if (err != ERR_BUSY) {
            ...
            if ((err == ERR_NONE) && (devCnt != 0U)) {
                for (i = 0; i < devCnt;
                     i++) /* Copy devices found form local Nfcb list into global
                             device list */
                {
                    gNfcDev.devList[gNfcDev.devCnt].type =
                 ...
                 Resolution */
    {
        ...
        if (!gNfcDev.isOperOngoing) {
            EXIT_ON_ERR(err, rfalNfcfPollerStartCollisionResolution(
                                 gNfcDev.disc.compMode,
                                 (gNfcDev.disc.devLimit - gNfcDev.devCnt),
                                 nfcfDevList, &devCnt));
        ...
        if (err != ERR_BUSY) {
            ...
            if ((err == ERR_NONE) && (devCnt != 0U)) {
                for (i = 0; i < devCnt;
                     i++) /* Copy devices found form local Nfcf list into global
                             device list */
                {
                    gNfcDev.devList[gNfcDev.devCnt].type =
                 ...
                 Resolution */
    {
        ...
        err = rfalNfcvPollerCollisionResolution(
            ...
            nfcvDevList, &devCnt);
        if ((err == ERR_NONE) && (devCnt != 0U)) {
            for (i = 0; i < devCnt; i++) /* Copy devices found form local Nfcf
                                            list into global device list */
            {
                gNfcDev.devList[gNfcDev.devCnt].type =
...
rfalNfcPollActivation(uint8_t devIt)
{
    ...
    case RFAL_NFC_LISTEN_TYPE_NFCA:
                ...
                .dev.nfca.isSleep) /* Check if desired device is in Sleep */
        {
            if (!gNfcDev.isOperOngoing) {
                /* Wake up all cards  */
                EXIT_ON_ERR(err, rfalNfcaPollerCheckPresence(
                                     RFAL_14443A_SHORTFRAME_CMD_WUPA,
                                     &gNfcDev.sensRes));

                /* Select specific device */
                EXIT_ON_ERR(err, rfalNfcaPollerStartSelect(
                ...
                /* In case multiple NFC-A devices are present, when
                   activating/waking a device that is sleeping (not the last
                   one) will make the active one to go back to IDLE. Marking it
                   as in sleep (Activity 2.2  9.4.4 Optional Symbol 2) will
                   ensure that gets correctly activated afterwards */
                for (devIdx = 0; devIdx < gNfcDev.devCnt; devIdx++) {
                    if (gNfcDev.devList[devIdx].type ==
                        RFAL_NFC_LISTEN_TYPE_NFCA) {
        ...
        case RFAL_NFCA_T4T: /* Device supports ISO-DEP */
            ...
            if (!gNfcDev.isOperOngoing) {
                ...
                EXIT_ON_ERR(err, rfalIsoDepPollAStartActivation(
        ...
        case RFAL_NFCA_NFCDEP: /* Device supports NFC-DEP */
            ...
            EXIT_ON_ERR(err, rfalNfcNfcDepActivate(&gNfcDev.devList[devIt],
    ...
    case RFAL_NFC_LISTEN_TYPE_NFCB:
        ...
        if (!gNfcDev.isTechInit) {
            ...
            {
                /* Wake up all cards. SENSB_RES may return collision but the
                 * NFCID0 is available to explicitly select NFC-B card via
                 * ATTRIB; so error will be ignored here */
        ...
        if (gNfcDev.devList[devIt].dev.nfcb.isSleep) /* Check if desired device
                                                        is still in Sleep */
        {
            /* Wake up all cards. SENSB_RES may return collision but the NFCID0
             * is available to explicitly select NFC-B card via ATTRIB; so error
        ...
        if ((gNfcDev.devList[devIt].dev.nfcb.sensbRes.protInfo.FsciProType &
            ...
            if (!gNfcDev.isOperOngoing) {
                ...
                EXIT_ON_ERR(err, rfalIsoDepPollBStartActivation(
                                     gNfcDev.disc.isoDepFS, RFAL_ISODEP_NO_DID,
    ...
    case RFAL_NFC_LISTEN_TYPE_NFCV:
        ...
        /* Set NFCID */
        gNfcDev.devList[devIt].nfcid =
            gNfcDev.devList[devIt].dev.nfcv.InvRes.UID;
    ...
    gNfcDev.activeDev =
        &gNfcDev
             .devList[devIt]; /* Assign active device to be used further on */
    return ERR_NONE;
}
...
rfalNfcListenActivation(void)
{
    ...
    case RFAL_LM_STATE_ACTIVE_Ax:
        ...
        if (isDataRcvd) /* Check if Reader/Initator has sent some data */
        {
            ...
            if (rfalNfcaListenerIsSleepReq(
                    gNfcDev.rxBuf.rfBuf,
                    rfalConvBitsToBytes(gNfcDev.rxLen))) /* Check if received
                                                            data is a SLP_REQ */
            {
            ...
            /* Check if received data is a valid RATS */
                ...
                rxParam.rxBuf =
                    (rfalIsoDepBufFormat *)&gNfcDev.rxBuf
                        .isoDepBuf; /*  PRQA S 0310 # MISRA 11.3 - Intentional
                                       safe cast to avoiding large buffer
                                       duplication */
                ...
                EXIT_ON_ERR(ret, rfalIsoDepListenStartActivation(
    ...
    case RFAL_LM_STATE_READY_F: /* NFC-F CE activation */
        ...
        if (isDataRcvd) /* Wait for the first received data */
        {
            ...
            {
                rfalListenSetState(
                    RFAL_LM_STATE_CARDEMU_3); /* First data already received -
                                                 set T3T CE */
            }
        }
        return ERR_BUSY;
    ...
    case RFAL_LM_STATE_TARGET_F:
        ...
        if (ret == ERR_NONE) {
            gNfcDev.devList->rfInterface = RFAL_NFC_INTERFACE_NFCDEP;
            gNfcDev.devList->nfcid = gNfcDev.devList->proto.nfcDep.activation
                                         .Initiator.ATR_REQ.NFCID3;
    ...
    case RFAL_LM_STATE_IDLE: /* AP2P activation */
        if (isDataRcvd)      /* Check if Reader/Initator has sent some data */
        {
                ...
                0U) /* Check if AP2P is enabled */
            {
                ...
                if (rfalNfcDepIsAtrReq(
                        &gNfcDev.rxBuf.rfBuf[hdrLen],
                        (rfalConvBitsToBytes(gNfcDev.rxLen) - hdrLen), NULL)) {
                    gNfcDev.devList->type = RFAL_NFC_POLL_TYPE_AP2P;
                    rfalSetMode((RFAL_MODE_LISTEN_ACTIVE_P2P), bitRate,
                                bitRate);
                      ...
                      const uint8_t *atrReq, uint16_t atrReqLen)
{
    ...
    /* If we are in Listen mode */
        ...
        ST_MEMCPY(targetParam.nfcid3, (uint8_t *)gNfcDev.disc.nfcid3,
        ...
        actvParams.rxBuf =
            (rfalNfcDepBufFormat *)&gNfcDev.rxBuf
                .nfcDepBuf; /*  PRQA S 0310 # MISRA 11.3 - Intentional safe cast
...
rfalNfcDeactivation(void)
{
    ...
    if (gNfcDev.activeDev != NULL) {
        if (rfalNfcIsRemDevListener(
                gNfcDev.activeDev->type)) /* Listen mode no additional
                                             deactivation to be performed*/
        {
