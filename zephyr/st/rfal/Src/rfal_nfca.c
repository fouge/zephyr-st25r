...
#define RFAL_FEATURE_NFCA                                                      \
    false /* NFC-A module configuration missing. Disabled by default */
#endif
...
#define rfalNfcaSelPar(nBy, nbi)                                               \
    (uint8_t)((((nBy) << 4U) & 0xF0U) |                                        \
              ((nbi)&0x0FU)) /*!< Calculates SEL_PAR with the bytes/bits to be \
                                sent */
...
typedef struct {
    ...
    rfalNfcaFColResState
        fState; /*!< Full Collision Resolution state (Full CR)               */
    uint8_t
...
*/
static uint8_t
rfalNfcaCalculateBcc(const uint8_t *buf, uint8_t bufLen);
static ReturnCode
rfalNfcaPollerStartSingleCollisionResolution(uint8_t devLimit,
                                             bool *collPending,
                                             rfalNfcaSelRes *selRes,
                                             uint8_t *nfcId1,
                                             uint8_t *nfcId1Len);
static ReturnCode
rfalNfcaPollerGetSingleCollisionResolutionStatus(void);
...
rfalNfcaPollerGetSingleCollisionResolutionStatus(void)
{
    ...
    case RFAL_NFCA_CR_SDD:
        ...
        if ((ret == ERR_TIMEOUT) && (gNfca.CR.devLimit == 0U) &&
            (gNfca.CR.retries != 0U)) {
            gNfca.CR.retries--;
            platformTimerDestroy(gNfca.CR.tmrFDT);
        ...
        if ((ret == ERR_TIMEOUT) && (gNfca.CR.backtrackCnt != 0U) &&
            ...
            rfalNfcaPollerCheckPresence(RFAL_14443A_SHORTFRAME_CMD_REQA,
                                        &sensRes);
            ...
            collBit =
                (uint8_t)((0U == collBit) ? 1U
                                          : 0U); // invert the collision bit
        ...
        if (ret == ERR_RF_COLLISION) {
            ...
            if (((gNfca.CR.bytesTxRx + ((gNfca.CR.bitsTxRx != 0U) ? 1U : 0U)) >
                ...
                (gNfca.CR.backtrackCnt !=
                 0U)) { /* Collision in BCC: Anticollide only UID part */
                gNfca.CR.backtrackCnt--;
                gNfca.CR.bytesTxRx =
                    (RFAL_NFCA_CASCADE_1_UID_LEN + RFAL_NFCA_SDD_REQ_LEN) - 1U;
                gNfca.CR.bitsTxRx = 7;
                collBit =
                    (uint8_t)(((uint8_t *)&gNfca.CR
                                   .selReq)[gNfca.CR.bytesTxRx] &
                              (1U
                               << gNfca.CR
                                      .bitsTxRx)); /* Not a real collision,
                                                      extract the actual bit for
                                                      the subsequent code */
            }

            if ((gNfca.CR.devLimit == 0U) && (!(*gNfca.CR.collPend))) {
                /* Activity 1.0 & 1.1  9.3.4.12: If CON_DEVICES_LIMIT has a
                 * value of 0, then NFC Forum Device is configured to perform
        ...
        /* If collisions are to be reported check whether the response is
         * complete */
    ...
    case RFAL_NFCA_CR_SEL:
        ...
        if (*gNfca.CR.selReq.nfcid1 == RFAL_NFCA_SDD_CT) {
            /* Cascade Tag present, store nfcid1 bytes (excluding cascade tag)
             * and continue for next CL */
            ST_MEMCPY(
            ...
            *gNfca.CR.nfcId1Len +=
            ...
            ST_MEMCPY(&gNfca.CR.nfcId1[*gNfca.CR.nfcId1Len],
...
/*******************************************************************************/
ReturnCode
rfalNfcaPollerCheckPresence(rfal14443AShortFrameCmd cmd,
                            rfalNfcaSensRes *sensRes)
{
    ReturnCode ret;
    uint16_t rcvLen;

    /* Digital 1.1 6.10.1.3  For Commands ALL_REQ, SENS_REQ, SDD_REQ, and
     * SEL_REQ, the NFC Forum Device      * MUST treat receipt of a Listen Frame
     * at a time after FDT(Listen, min) as a Timeour Error */
...
/*******************************************************************************/
ReturnCode
rfalNfcaPollerStartTechnologyDetection(rfalComplianceMode compMode,
                                       rfalNfcaSensRes *sensRes)
{
...
/*******************************************************************************/
ReturnCode
rfalNfcaPollerGetTechnologyDetectionStatus(void)
{
    ReturnCode ret;
...
/*******************************************************************************/
ReturnCode
rfalNfcaPollerSingleCollisionResolution(uint8_t devLimit, bool *collPending,
                                        rfalNfcaSelRes *selRes, uint8_t *nfcId1,
                                        uint8_t *nfcId1Len)
{
    ReturnCode ret;

    EXIT_ON_ERR(ret, rfalNfcaPollerStartSingleCollisionResolution(
                                           ...
                                           uint8_t *devCnt)
{
    ...
    *devCnt = 0;
    ...
    /*******************************************************************************/
    /* Send ALL_REQ before Anticollision if a Sleep was sent before Activity 1.1
     * 9.3.4.1 and EMVco 2.6  9.3.2.1 */
    if (compMode != RFAL_COMPLIANCE_MODE_ISO) {
        ...
        if (ret != ERR_NONE) {
            if ((compMode == RFAL_COMPLIANCE_MODE_EMV) ||
                ((ret != ERR_RF_COLLISION) && (ret != ERR_CRC) &&
                 (ret != ERR_FRAMING) && (ret != ERR_PAR) &&
                 (ret != ERR_INCOMPLETE_BYTE))) {
                return ret;
            }
     ...
     * first device When only one device is detected it's not woken up then
     * we'll have no SENS_RES (ATQA) */
...
#if RFAL_FEATURE_T1T
    /*******************************************************************************/
    /* Only check for T1T if previous SENS_RES was received without a
     * transmission  * error. When collisions occur bits in the SENS_RES may
     * look like a T1T        */
    /* If T1T Anticollision is not supported  Activity 1.1  9.3.4.3 */
    if (rfalNfcaIsSensResT1T(&nfcaDevList->sensRes) && (devLimit != 0U) &&
        (ret == ERR_NONE) && (compMode != RFAL_COMPLIANCE_MODE_EMV)) {
        ...
        *devCnt = 1U;
        ...
        ST_MEMCPY(&nfcaDevList->nfcId1, &nfcaDevList->ridRes.uid,
                  RFAL_NFCA_CASCADE_1_UID_LEN);
...
/*******************************************************************************/
ReturnCode
rfalNfcaPollerGetFullCollisionResolutionStatus(void)
{
    ReturnCode ret;
    uint8_t newDevType;
    ...
    case RFAL_NFCA_CR_FULL_RESTART: /*  PRQA S 2003 # MISRA 16.3 - Intentional
        ...
        /* PRQA S 4342 1 # MISRA 10.5 - Guaranteed that no invalid enum values
         * are created: see guard_eq_RFAL_NFCA_T2T, .... */
    ...
    case RFAL_NFCA_CR_FULL_SLPCHECK:
        ...
        if (ret == ERR_TIMEOUT) {
            ...
            if ((*gNfca.CR.devCnt < gNfca.CR.devLimit) &&
                ...
                EXIT_ON_ERR(
                    ...
                    rfalNfcaPollerStartSingleCollisionResolution(
                        ...
                        (uint8_t *)&gNfca.CR.nfcaDevList[*gNfca.CR.devCnt]
                            .nfcId1,
                                      ...
                                      uint8_t *devCnt)
{
    ...
    EXIT_ON_ERR(ret, rfalNfcaPollerStartFullCollisionResolution(
...
ReturnCode
rfalNfcaPollerSleepFullCollisionResolution(uint8_t devLimit,
                                           rfalNfcaListenDevice *nfcaDevList,
                                           uint8_t *devCnt)
{
    bool firstRound;
...
/*******************************************************************************/
ReturnCode
rfalNfcaPollerSelect(const uint8_t *nfcid1, uint8_t nfcidLen,
                     rfalNfcaSelRes *selRes)
{
    ReturnCode ret;
...
/*******************************************************************************/
ReturnCode
rfalNfcaPollerStartSelect(const uint8_t *nfcid1, uint8_t nfcidLen,
                          rfalNfcaSelRes *selRes)
{
...
/*******************************************************************************/
ReturnCode
rfalNfcaPollerGetSelectStatus(void)
{
    ReturnCode ret;
        ...
        if (gNfca.SEL.cascadeLv <= gNfca.SEL.fCascadeLv) {
            ...
            if (gNfca.SEL.fCascadeLv != gNfca.SEL.cascadeLv) {
                *selReq.nfcid1 = RFAL_NFCA_SDD_CT;
                ST_MEMCPY(&selReq.nfcid1[RFAL_NFCA_SDD_CT_LEN],
                          &gNfca.SEL.nfcid1[gNfca.SEL.nfcidOffset],
                          (RFAL_NFCA_CASCADE_1_UID_LEN - RFAL_NFCA_SDD_CT_LEN));
                gNfca.SEL.nfcidOffset +=
                    (RFAL_NFCA_CASCADE_1_UID_LEN - RFAL_NFCA_SDD_CT_LEN);
            } else {
                ST_MEMCPY(selReq.nfcid1,
                          &gNfca.SEL.nfcid1[gNfca.SEL.nfcidOffset],
                          RFAL_NFCA_CASCADE_1_UID_LEN);
            }
...
/*******************************************************************************/
ReturnCode
rfalNfcaPollerGetSleepStatus(void)
{
    ReturnCode ret;
    ...
    /* ISO14443-3 6.4.3  HLTA - If PICC responds with any modulation during 1 ms
       ...
       Digital 2.0  6.9.2.1 & EMVCo 3.0  5.6.2.1 - consider the HLTA command
       always acknowledged No check to be compliant with NFC and EMVCo, and to
       improve interoprability (Kovio RFID Tag)
    */
    EXIT_ON_BUSY(ret, rfalGetTransceiveStatus());
...
/*******************************************************************************/
bool
rfalNfcaListenerIsSleepReq(const uint8_t *buf, uint16_t bufLen)
{
