...
#define RFAL_NFCDEP_HEADER                                                     \
    (RFAL_NFCDEP_CMDTYPE_LEN +                                                 \
     RFAL_NFCDEP_CMD_LEN) /*!< NFCIP header length           */
...
#define RFAL_NFCDEP_WT_DELTA                                                   \
    (16U - RFAL_NFCDEP_WT_DELTA_ADJUST) /*!< NFC-DEP dWRT (adjusted)           \
                                           Digital 2.0 B.10  */
...
#define RFAL_NFCDEP_WT_INI_DEFAULT                                             \
    RFAL_NFCDEP_WT_INI_MAX /*!< WT Initiator default value                     \
                              Digital 1.0 14.6.3.8        */
...
#define RFAL_NFCDEP_RWT_INI_MAX                                                \
    rfalNfcDepWT2RWT(                                                          \
        RFAL_NFCDEP_WT_INI_MAX) /*!< RWT Initiator maximum value   */
...
#define RFAL_NFCDEP_RWT_TRG_MAX                                                \
    rfalNfcDepWT2RWT(                                                          \
        RFAL_NFCDEP_WT_TRG_MAX) /*!< RWT Initiator maximum value         */
...
#define rfalNfcDepBRS2DRI(brs)                                                 \
    (uint8_t)(                                                                 \
        (uint8_t)(brs)&RFAL_NFCDEP_BRS_Dx_MASK) /*!< Returns the DRI value     \
                                                   from the given BRS byte */
#define rfalNfcDepBRS2DSI(brs)                                                 \
    (uint8_t)(((uint8_t)(brs) >> RFAL_NFCDEP_BRS_DSI_POS) &                    \
              RFAL_NFCDEP_BRS_Dx_MASK) /*!< Returns the DSI value from the     \
                                          given BRS byte */

#define rfalNfcDepPP2LR(PPx)                                                   \
    (((uint8_t)(PPx)&RFAL_NFCDEP_PP_LR_MASK) >>                                \
     RFAL_NFCDEP_PP_LR_SHIFT) /*!< Returns the LR value from the given PPx     \
                                 byte  */
...
#define rfalNfcDepLR2FS(LRx)                                                   \
    (uint16_t)(MIN((RFAL_NFCDEP_FS_VAL_MIN * ((uint16_t)(LRx) + 1U)),          \
...
enum {
    ...
    RFAL_NFCDEP_Dx_64_6780 =
        RFAL_BR_6780 /*!< Divisor D = 64 : bit rate = 6780     */
};
...
typedef struct {
    ...
    uint8_t
        nfcid[RFAL_NFCDEP_NFCID3_LEN]; /*!< Pointer to the NFCID to be used */
...
typedef struct {
    ...
    uint8_t pdu[RFAL_FEATURE_NFC_DEP_PDU_MAX_LEN];   /*!< Complete PDU/Payload
                                                        buffer             */
