...
*/
#define RFAL_VERSION                                                           \
    0x020600U /*!< RFAL Current Version: v2.6.0                      */
...
#define RFAL_LM_MASK_ACTIVE_P2P                                                \
    ((uint32_t)1U << (uint8_t)                                                 \
         RFAL_MODE_LISTEN_ACTIVE_P2P) /*!< Bitmask for Listen Mode enabling    \
                                         AP2P    */
...
#define rfalIsModePassiveComm(md)                                              \
    (!rfalIsModeActiveComm(                                                    \
        md)) /*!< Checks if mode md is Passive Communication */
...
#define rfalConv8fcTo1fc(t)                                                    \
    (uint32_t)((uint32_t)(t)*RFAL_1FC_IN_8FC) /*!< Converts the given t from   \
                                                 8/fc to 1/fc     */
...
#define rfalConv64fcTo1fc(t)                                                   \
    (uint32_t)((uint32_t)(t)*RFAL_1FC_IN_64FC) /*!< Converts the given t from  \
                                                  64/fc to 1/fc    */
...
#define rfalConv512fcTo1fc(t)                                                  \
    (uint32_t)((uint32_t)(t)*RFAL_1FC_IN_512FC) /*!< Converts the given t from \
                                                   512/fc to 1/fc   */
...
#define rfalConv2048fcTo1fc(t)                                                 \
    (uint32_t)((uint32_t)(t)*RFAL_1FC_IN_2048FC) /*!< Converts the given t     \
                                                    from 2048/fc to 1/fc  */
...
#define rfalConv4096fcTo1fc(t)                                                 \
    (uint32_t)((uint32_t)(t)*RFAL_1FC_IN_4096FC) /*!< Converts the given t     \
                                                    from 4096/fc to 1/fc  */

#define rfalConv1fcToMs(t)                                                     \
    (uint32_t)((uint32_t)(t) /                                                 \
               RFAL_1MS_IN_1FC) /*!< Converts the given t from 1/fc to ms */
#define rfalConvMsTo1fc(t)                                                     \
    (uint32_t)((uint32_t)(t)*RFAL_1MS_IN_1FC) /*!< Converts the given t from   \
                                                 ms to 1/fc       */
...
#define rfalLogE(...)                                                          \
    LOG_ERR(__VA_ARGS__) /*!< Macro for the error log method */
#define rfalLogW(...)                                                          \
...
#define RFAL_FDT_LISTEN_NFCA_POLLER                                            \
    1172U /*!< FDTA,LISTEN,MIN (n=9) Last bit: Logic "1" - tnn,min/2           \
             Digital 1.1  6.10 ;  EMV CCP Spec Book D v2.01  4.8.1.3   */
#define RFAL_FDT_LISTEN_NFCB_POLLER                                            \
    1008U /*!< TR0B,MIN         Digital 1.1  7.1.3 & A.3  ; EMV CCP Spec Book  \
             D v2.01  4.8.1.3 & Table A.5                     */
...
enum {
    ...
    RFAL_TXRX_FLAGS_CRC_RX_KEEP =
        (1U << 1), /*!< Upon Reception keep the CRC in rxBuffer (reflected on
                      rcvd length)                    */
    RFAL_TXRX_FLAGS_CRC_RX_REMV =
        (0U << 1), /*!< Remove the CRC from rxBuffer */
    RFAL_TXRX_FLAGS_NFCIP1_ON =
        (1U << 2), /*!< Enable NFCIP1 mode: Add SB(F0) and LEN bytes during Tx
                      and skip SB(F0) byte during Rx */
    RFAL_TXRX_FLAGS_NFCIP1_OFF =
        (0U << 2), /*!< Disable NFCIP1 mode: do not append protocol bytes while
                      Tx nor skip while Rx          */
    RFAL_TXRX_FLAGS_AGC_OFF =
        (1U << 3), /*!< Disable Automatic Gain Control, improving multiple
                      devices collision detection        */
    RFAL_TXRX_FLAGS_AGC_ON = (0U << 3), /*!< Enable Automatic Gain Control,
                                           improving single device reception */
...
typedef enum {
    RFAL_14443A_SHORTFRAME_CMD_WUPA =
        0x52, /*!< ISO14443A WUPA / NFC-A ALL_REQ  */
    RFAL_14443A_SHORTFRAME_CMD_REQA =
        0x26 /*!< ISO14443A REQA / NFC-A SENS_REQ */
...
typedef enum {
    ...
    RFAL_WUM_STATE_ENABLED_WOKE =
        0x02, /*!< Wake-Up mode enabled and has received IRQ(s)*/
} rfalWumState;
 ...
 *
 *
 * \param[in]   buf        : reference to ANTICOLLISION command (with known UID
 *if any) to be sent (also out param) reception will be place on this buf after
 *bytesToSend \param[in]   bytesToSend: reference number of full bytes to be
 *sent (including CMD byte and SEL_PAR) if a collision occurs will contain the
 *number of clear bytes \param[in]   bitsToSend : reference to number of bits
 *(0-7) to be sent; and received (also out param) if a collision occurs will
 *indicate the number of clear bits (also out param) \param[out]  rxLength   :
 *reference to the return the received length \param[in]   fwt        : Frame
 *Waiting Time in 1/fc
 *
