...
#define RFAL_ISODEP_ATTRIB_REQ_PARAM1_DEFAULT                                  \
    (0x00U) /*!< Default values of Param 1 of ATTRIB_REQ                       \
               Digital 1.0  12.6.1.3-5   */
...
typedef enum {
    ...
    RFAL_ISODEP_FSXI_512 = 9,   /*!< Frame Size for Proximity Card Integer with
                                   512 bytes   ISO14443-3 Amd2 2012 */
...
typedef union { /*  PRQA S 0750 # MISRA 19.2 - Both members of the union will
    ...
    union {
        ...
        struct {
            rfalIsoDepAttribCmd
                ATTRIB;        /*!< ATTRIB request        (Listener mode)    */
...
typedef struct {
    ...
    rfalBitRate
        DSI; /*!< Bit Rate coding from Listener (PICC) to Poller (PCD) */
    rfalBitRate
        DRI;      /*!< Bit Rate coding from Poller (PCD) to Listener (PICC) */
 ...
 *  \return ERR_FRAMING      : Framing error detected
 *  \return ERR_PROTO        : Protocol error detected
 *  \return ERR_NONE         : No error, start of asynchronous operation
 *successful
 *****************************************************************************
 */
ReturnCode
