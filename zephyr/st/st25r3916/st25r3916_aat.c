...
*/
static ReturnCode
aatHillClimb(const struct st25r3916AatTuneParams *tuningParams,
             struct st25r3916AatTuneResult *tuningStatus);
static int32_t
aatGreedyDescent(uint32_t *f_min,
                 const struct st25r3916AatTuneParams *tuningParams,
                 struct st25r3916AatTuneResult *tuningStatus,
                 int32_t previousDir);
static int32_t
aatSteepestDescent(uint32_t *f_min,
                   const struct st25r3916AatTuneParams *tuningParams,
                   struct st25r3916AatTuneResult *tuningStatus,
                   int32_t previousDir, int32_t previousDir2);

static ReturnCode
aatMeasure(uint8_t serCap, uint8_t parCap, uint8_t *amplitude, uint8_t *phase,
           uint16_t *measureCnt);
static uint32_t
aatCalcF(const struct st25r3916AatTuneParams *tuningParams, uint8_t amplitude,
         uint8_t phase);
static ReturnCode
aatStepDacVals(const struct st25r3916AatTuneParams *tuningParams, uint8_t *a,
               uint8_t *b, int32_t dir);

/*******************************************************************************/
ReturnCode
st25r3916AatTune(const struct st25r3916AatTuneParams *tuningParams,
                 struct st25r3916AatTuneResult *tuningStatus)
{
    ReturnCode err;
             ...
             struct st25r3916AatTuneResult *tuningStatus)
{
    ...
    uint8_t amp, phs;
    struct st25r3916AatTuneParams tp =
        *tuningParams; // local copy to obey const
    ...
    do {
        direction = 0; /* Initially and after reducing step sizes we don't have
                          a previous direction */
        do {
            /* With the greedy step below always executed aftwards the
             * -direction does never need to be investigated */
            direction = aatSteepestDescent(&f_min, &tp, tuningStatus, direction,
                                           -direction);
            ...
            do {
                gdirection =
                    aatGreedyDescent(&f_min, &tp, tuningStatus, direction);
                if (tuningStatus->measureCnt > tp.measureLimit) {
                    err = ERR_OVERRUN;
                    break;
                }
                   ...
                   int32_t previousDir, int32_t previousDir2)
{
    ...
    for (i = -2; i <= 2; i++) {
        ...
        if (0U != aatStepDacVals(tuningParams, &a, &b,
                                 i)) { /* If stepping did not change the value,
                                          omit this direction */
            continue;
        }
               ...
               uint8_t *b, int32_t dir)
{
    ...
    /* We only get here if actual values have changed. In all other cases an
     * error is returned */
    *a = (uint8_t)aat_a;
    *b = (uint8_t)aat_b;
