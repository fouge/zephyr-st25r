...
#define RFAL_FEATURE_LISTEN_MODE                                               \
    false /* Listen Mode configuration missing. Disabled by default */
...
typedef struct {
    ...
    rfalTransceiveState
        lastState; /*!< Last transceive state (debug purposes)              */
    ReturnCode status; /*!< Current status/error of the transceive */

    rfalTransceiveContext
        ctx; /*!< The transceive context given by the caller          */
} rfalTxRx;
...
typedef struct {
    uint16_t expWL;        /*!< The amount of bytes expected to be Tx when a WL
                              interrupt occours                          */
...
typedef struct {
    ...
    rfalFeliCaPollRes
        pollResponses[RFAL_FELICA_POLL_MAX_SLOTS]; /* FeliCa Poll response
                                                      buffer (16 slots)  */
} rfalNfcfWorkingData;
...
typedef enum {
    FELICA_CMD_POLLING = 0x00, /*!< Felica Poll/REQC command (aka SENSF_REQ) to
                                  identify a card    */
    ...
    FELICA_CMD_READ_WITHOUT_ENCRYPTION =
        0x06, /*!< read Block Data from a Service that requires no
                 authentication */
...
typedef union { /*  PRQA S 0750 # MISRA 19.2 - Both members are of the same
    ...
    uint8_t PTMem_A[ST25R3916_PTM_A_LEN]; /*!< PT_Memory area allocated for
                                             NFC-A configuration */
    uint8_t PTMem_F[ST25R3916_PTM_F_LEN]; /*!< PT_Memory area allocated for
                                             NFC-F configuration */
...
#define RFAL_ST25R3916_MRT_MAX_1FC                                             \
    rfalConv64fcTo1fc(0x00FFU) /*!< Max MRT steps in 1fc (0x00FF steps of      \
                                  64/fc   => 0x00FF * 4.72us = 1.2ms )      */
#define RFAL_ST25R3916_MRT_MIN_1FC                                             \
    rfalConv64fcTo1fc(0x0004U) /*!< Min MRT steps in 1fc ( 0<=mrt<=4 ; 4       \
                                  (64/fc)  => 0x0004 * 4.72us = 18.88us ) */
...
#define RFAL_FELICA_POLL_DELAY_TIME                                            \
    512U /*!<  FeliCa Poll Processing time is 2.417 ms ~512*64/fc Digital 1.1  \
            A4              */
#define RFAL_FELICA_POLL_SLOT_TIME                                             \
    256U /*!<  FeliCa Poll Time Slot duration is 1.208 ms ~256*64/fc           \
            Digital 1.1 A4           */
...
#define RFAL_ST25R3916_AAT_SETTLE                                              \
    5U /*!< Time in ms required for AAT pins and Osc to settle after en bit    \
          set             */
...
#define rfalGennTRFW(n)                                                        \
    ((uint8_t)(((n) + 1U) % 7U)) /*!< Generate next n*TRFW used for RFCA:      \
                                    modulo a prime to avoid alias effects */

#define rfalCalcNumBytes(nBits)                                                \
    (((uint32_t)(nBits) + 7U) / 8U) /*!< Returns the number of bytes required  \
                                       to fit given the number of bits */
...
#define rfalIsIncompleteByteError(error)                                       \
    (((error) >= ERR_INCOMPLETE_BYTE) &&                                       \
     ((error) <= ERR_INCOMPLETE_BYTE_07)) /*!< Checks if given error is a      \
                                             Incomplete error */
...
#define rfalConvBR2ACBR(b)                                                     \
    (((rfalAdjACBR((b))) << RFAL_ANALOG_CONFIG_BITRATE_SHIFT) &                \
     RFAL_ANALOG_CONFIG_BITRATE_MASK) /*!< Converts ST25R391x Bit rate to      \
                                         Analog Configuration bit rate id */
...
#define rfalRunBlocking(e, fn)                                                 \
    do {                                                                       \
        (e) = (fn);                                                            \
        rfalWorker();                                                          \
    } while ((e) == ERR_BUSY) /*!< Macro used for the blocking operations */
...
static void
rfalTransceiveTx(void);
static void
rfalTransceiveRx(void);
static ReturnCode
rfalTransceiveRunBlockingTx(void);
static void
rfalPrepareTransceive(void);
static void
rfalCleanupTransceive(void);
static void
rfalErrorHandling(void);

static ReturnCode
rfalRunTransceiveWorker(void);
#if RFAL_FEATURE_LISTEN_MODE
static ReturnCode
rfalRunListenModeWorker(void);
#endif /* RFAL_FEATURE_LISTEN_MODE */
#if RFAL_FEATURE_WAKEUP_MODE
static void
rfalRunWakeUpModeWorker(void);
static uint16_t
rfalWakeUpModeFilter(uint16_t curRef, uint16_t curVal, uint8_t weight);
#endif /* RFAL_FEATURE_WAKEUP_MODE */

static void
rfalFIFOStatusUpdate(void);
static void
rfalFIFOStatusClear(void);
static bool
rfalFIFOStatusIsMissingPar(void);
static bool
rfalFIFOStatusIsIncompleteByte(void);
static uint16_t
rfalFIFOStatusGetNumBytes(void);
static uint8_t
rfalFIFOGetNumIncompleteBits(void);
...
rfalInitialize(void)
{
    ...
    /*******************************************************************************/
    /* Perform Automatic Calibration (if configured to do so). * Registers set
     * by rfalSetAnalogConfig will tell rfalCalibrate what to perform*/
    rfalCalibrate();
...
rfalCalibrate(void)
{
    ...
    /* Automatic regulator adjustment only performed if not set manually on
     * Analog Configs */
...
/*******************************************************************************/
ReturnCode
rfalAdjustRegulators(uint16_t *result)
{
...
/*******************************************************************************/
void
rfalSetUpperLayerCallback(rfalUpperLayerCallback pFunc)
{
...
/*******************************************************************************/
void
rfalSetPreTxRxCallback(rfalPreTxRxCallback pFunc)
{
...
/*******************************************************************************/
void
rfalSetSyncTxRxCallback(rfalSyncTxRxCallback pFunc)
{
...
/*******************************************************************************/
void
rfalSetPostTxRxCallback(rfalPostTxRxCallback pFunc)
{
...
/*******************************************************************************/
void
rfalSetObsvMode(uint32_t txMode, uint32_t rxMode)
{
...
/*******************************************************************************/
void
rfalGetObsvMode(uint8_t *txMode, uint8_t *rxMode)
{
    if (txMode != NULL) {
...
/*******************************************************************************/
ReturnCode
rfalSetMode(rfalMode mode, rfalBitRate txBR, rfalBitRate rxBR)
{
    ...
    case RFAL_MODE_POLL_NFCB:
        ...
        st25r3916ChangeRegisterBits(
            ...
            (ST25R3916_REG_ISO14443B_1_egt_mask |
             ST25R3916_REG_ISO14443B_1_sof_mask |
    ...
    case RFAL_MODE_POLL_ACTIVE_P2P:
        ...
        /* Set PPon2 timer with the max time between our field Off and other
         * peer field On : Tadt + (n x Trfw)    */
    ...
    case RFAL_MODE_LISTEN_ACTIVE_P2P:
        ...
        st25r3916WriteRegister(ST25R3916_REG_MODE,
                               (ST25R3916_REG_MODE_targ_targ |
                                ST25R3916_REG_MODE_om_targ_nfcip |
                                ST25R3916_REG_MODE_nfc_ar_eof));
    ...
    case RFAL_MODE_LISTEN_NFCF:
        ...
        st25r3916WriteRegister(ST25R3916_REG_MODE,
                               (ST25R3916_REG_MODE_targ |
                                ST25R3916_REG_MODE_om_targ_nfcf |
                                ST25R3916_REG_MODE_nfc_ar_off));
    ...
    gRFAL.state = ((gRFAL.state < RFAL_STATE_MODE_SET) ? RFAL_STATE_MODE_SET
                                                       : gRFAL.state);
    gRFAL.mode = mode;
...
rfalSetBitRate(rfalBitRate txBR, rfalBitRate rxBR)
{
    ...
    case RFAL_MODE_POLL_NFCA_T1T:
        ...
        rfalSetAnalogConfig((RFAL_ANALOG_CONFIG_TECH_CHIP |
                             RFAL_ANALOG_CONFIG_CHIP_POLL_COMMON));
    ...
    case RFAL_MODE_POLL_PICOPASS:
...

        {
            ...
            /* MISRA 11.3 - Cannot point directly into different object type,
             * copy to local var */
...
/*******************************************************************************/
ReturnCode
rfalGetBitRate(rfalBitRate *txBR, rfalBitRate *rxBR)
{
...
rfalSetErrorHandling(rfalEHandling eHandling)
{
    ...
    case RFAL_ERRORHANDLING_EMD:
        /* MISRA 16.4: no empty default statement (in case RFAL_SW_EMD is
         * defined) */
#ifndef RFAL_SW_EMD
...
/*******************************************************************************/
void
rfalSetFDTPoll(uint32_t FDTPoll)
{
...
/*******************************************************************************/
void
rfalSetFDTListen(uint32_t FDTListen)
{
...
/*******************************************************************************/
ReturnCode
rfalFieldOnAndStartGT(void)
{
    ReturnCode ret;

    /* Check if RFAL has been initialized (Oscillator should be running) and
     * also if a direct register access has been performed and left the
     * Oscillator Off */
    ...
    if ((!st25r3916IsTxEnabled()) || (!gRFAL.field)) {
        ...
        /* Only turn on Receiver and Transmitter if field was successfully
         * turned On */
        if (gRFAL.field) {
    ...
    if ((gRFAL.timings.GT != RFAL_TIMING_NONE)) {
        ...
        rfalTimerStart(gRFAL.tmr.GT,
                       rfalConv1fcToMs(
                           MAX((gRFAL.timings.GT), RFAL_ST25R3916_GT_MIN_1FC)));
    }
...
/*******************************************************************************/
ReturnCode
rfalStartTransceive(const rfalTransceiveContext *ctx)
{
    ...
    if (gRFAL.state >= RFAL_STATE_MODE_SET) {
        /*******************************************************************************/
        /* Check whether the field is already On, otherwise no TXE will be
         * received  */
        ...
        if (gRFAL.timings.FDTListen != RFAL_TIMING_NONE) {
            ...
            /* If Correlator is being used further adjustment is required for
             * NFCB */
            if ((st25r3916CheckReg(ST25R3916_REG_AUX,
                                   ST25R3916_REG_AUX_dis_corr, 0x00U)) &&
                (gRFAL.mode == RFAL_MODE_POLL_NFCB)) {
                FxTAdj += (uint32_t)
                    RFAL_FDT_LISTEN_B_ADJT_CORR; /* Reduce FDT(Listen) */
                st25r3916SetRegisterBits(
                    ST25R3916_REG_CORR_CONF1,
                    ST25R3916_REG_CORR_CONF1_corr_s3); /* Ensure BPSK start to
                                                          33 pilot pulses */
                st25r3916ChangeRegisterBits(
            ...
            /* Set Minimum FDT(Listen) in which PICC is not allowed to send a
             * response */
            st25r3916WriteRegister(
                ST25R3916_REG_MASK_RX_TIMER,
                (uint8_t)rfalConv1fcTo64fc(
                    (FxTAdj > gRFAL.timings.FDTListen)
                        ? RFAL_ST25R3916_MRT_MIN_1FC
                        : (gRFAL.timings.FDTListen - FxTAdj)));
        }

        /*******************************************************************************/
        /* FDT Poll will be loaded in rfalPrepareTransceive() once the previous
         * was expired */

        /*******************************************************************************/
        if ((gRFAL.TxRx.ctx.fwt != RFAL_FWT_NONE) &&
            (gRFAL.TxRx.ctx.fwt != 0U)) {
            ...
            if ((gRFAL.mode == RFAL_MODE_POLL_NFCF) ||
                ...
                FxTAdj += (uint32_t)((gRFAL.txBR == RFAL_BR_212)
                                         ? RFAL_FWT_F_212_ADJUSTMENT
                                         : RFAL_FWT_F_424_ADJUSTMENT);
            }

            /* Ensure that the given FWT doesn't exceed NRT maximum */
            gRFAL.TxRx.ctx.fwt =
                MIN((gRFAL.TxRx.ctx.fwt + FxTAdj), RFAL_ST25R3916_NRT_MAX_1FC);

            /* Set FWT in the NRT */
            st25r3916SetNoResponseTime(rfalConv1fcTo64fc(gRFAL.TxRx.ctx.fwt));
        } else {
            /* Disable NRT, no NRE will be triggered, therefore wait endlessly
             * for Rx */
        ...
        if ((RFAL_MODE_POLL_NFCV == gRFAL.mode) ||
            ...
            gRFAL.TxRx.ctx.rxBuf = ((gRFAL.nfcvData.origCtx.rxBuf != NULL)
                                        ? gRFAL.nfcvData.codingBuffer
                                        : NULL);
            gRFAL.TxRx.ctx.rxBufLen = (uint16_t)rfalConvBytesToBits(
                sizeof(gRFAL.nfcvData.codingBuffer));
            ...
            /* In NFCV a TxRx with a valid txBuf and txBufSize==0 indicates to
             * send an EOF */
        ...
        if (st25r3916CheckReg(ST25R3916_REG_AUX_MOD,
            ...
            /* If ST25R3916 with AWS set again the current mode to reload AWS
             * config */
        ...
        if ((gRFAL.TxRx.ctx.txBuf == NULL) || (gRFAL.TxRx.ctx.txBufLen == 0U)) {
            ...
            if (rfalIsModeActiveComm(gRFAL.mode)) {
                /* Disable our field upon a Rx reEnable, and start PPON2
                 * manually */
                st25r3916TxOff();
            ...
            /* Start NRT manually, if FWT = 0 (wait endlessly for Rx) chip will
             * ignore anyhow */
...
/*******************************************************************************/
ReturnCode
rfalTransceiveBlockingTx(uint8_t *txBuf, uint16_t txBufLen, uint8_t *rxBuf,
                         ...
                         uint32_t fwt)
{
    ...
    rfalCreateByteFlagsTxRxContext(ctx, txBuf, txBufLen, rxBuf, rxBufLen,
                                   actLen, flags, fwt);
...
/*******************************************************************************/
ReturnCode
rfalTransceiveBlockingTxRx(uint8_t *txBuf, uint16_t txBufLen, uint8_t *rxBuf,
                           uint16_t rxBufLen, uint16_t *actLen, uint32_t flags,
                           uint32_t fwt)
{
    ReturnCode ret;

    EXIT_ON_ERR(ret, rfalTransceiveBlockingTx(txBuf, txBufLen, rxBuf, rxBufLen,
                                              actLen, flags, fwt));
    ...
    if (gRFAL.state == RFAL_STATE_TXRX) {
        ...
        if (gRFAL.tmr.txRx != RFAL_TIMING_NONE) {
            if (rfalTimerisExpired(gRFAL.tmr.txRx)) {
                /* If sanity timer has expired abort ongoing transceive and
                 * signal error */
...
/*******************************************************************************/
ReturnCode
rfalGetTransceiveRSSI(uint16_t *rssi)
{
    uint16_t amRSSI;
    uint16_t pmRSSI;
    bool isSumMode;
    ...
    if (isSumMode) {
         ...
         * performance issues sqrt is not enabled by default. Using a less
         * accuracy    * accurate aproach such as: average, max value, etc */
...
rfalErrorHandling(void)
{
    ...
    if (gRFAL.conf.eHandling == RFAL_ERRORHANDLING_EMD) {
        ...
        /*******************************************************************************/
        /* EMD Handling - Digital 2.1  4.1.1.1 ; EMVCo 3.0  4.9.2 ; ISO
         * 14443-3  8.3   */
        ...
        /* Ensure FIFO contains some byte as the FIFO might be empty upon
         * Framing errors */
        if ((fifoBytesToRead > 0U) && rxHasIncParError) {
            fifoBytesToRead--;
        }

        if (((gRFAL.fifo.bytesTotal + fifoBytesToRead) <
             RFAL_EMVCO_RX_MAXLEN) &&
            ((gRFAL.TxRx.status == ERR_RF_COLLISION) ||
             (gRFAL.TxRx.status == ERR_FRAMING) ||
             (gRFAL.TxRx.status == ERR_PAR) || (gRFAL.TxRx.status == ERR_CRC) ||
             rxHasIncParError)) {
            /* Ignore this reception, ReEnable receiver which also clears the
             * FIFO */
            st25r3916ExecuteCommand(ST25R3916_CMD_UNMASK_RECEIVE_DATA);
    ...
    if (gRFAL.mode == RFAL_MODE_POLL_NFCA) {
        ...
        if ((gRFAL.TxRx.status == ERR_PAR) || (gRFAL.TxRx.status == ERR_CRC)) {
            if ((rfalFIFOStatusIsIncompleteByte()) &&
                ...
                st25r3916ReadFifo((uint8_t *)(gRFAL.TxRx.ctx.rxBuf),
                                  fifoBytesToRead);
...
rfalPrepareTransceive(void)
{
    ...
    if (gRFAL.timings.FDTPoll != RFAL_TIMING_NONE) {
        /* In Passive communications General Purpose Timer is used to measure
         * FDT Poll */
    ...
    /* Check if Parity check is to be skipped and to keep the parity + CRC bits
     * in FIFO */
    ...
    if ((gRFAL.TxRx.ctx.flags & (uint32_t)RFAL_TXRX_FLAGS_CRC_RX_MANUAL) !=
        0U) {
        st25r3916SetRegisterBits(ST25R3916_REG_AUX,
                                 ST25R3916_REG_AUX_no_crc_rx);
    } else {
        st25r3916ClrRegisterBits(ST25R3916_REG_AUX,
                                 ST25R3916_REG_AUX_no_crc_rx);
    }
    ...
    if (rfalIsModePassiveListen(gRFAL.mode)) {
        maskInterrupts |=
            (ST25R3916_IRQ_MASK_EOF |
             ST25R3916_IRQ_MASK_WU_F); /* Enable external Field interrupts to
                                          detect Link Loss and SENF_REQ auto
                                          responses */
    }
    ...
    if (rfalIsModeActiveComm(gRFAL.mode)) {
        ...
        st25r3916ChangeRegisterBits(ST25R3916_REG_AUX,
                                    ST25R3916_REG_AUX_nfc_n_mask, 0);
    }
...
rfalTransceiveTx(void)
{
    ...
    case RFAL_TXRX_STATE_TX_PREP_TX: /*  PRQA S 2003 # MISRA 16.3 - Intentional
        ...
        if ((RFAL_MODE_POLL_NFCV == gRFAL.mode) ||
            (RFAL_MODE_POLL_PICOPASS == gRFAL.mode)) {
                ...
                st25r3916ExecuteCommand( ST25R3916_CMD_CLEAR_FIFO );
#endif
            /* Calculate the bytes needed to be Written into FIFO (a incomplete
             * byte will be added as 1byte) */
            ...
            ret = rfalIso15693VCDCode(
                ...
                MIN((uint16_t)ST25R3916_FIFO_DEPTH,
...
#endif /* RFAL_FEATURE_NFCV */
        {
            ...
            gRFAL.fifo.bytesWritten =
                MIN(gRFAL.fifo.bytesTotal, ST25R3916_FIFO_DEPTH);
        ...
        /*******************************************************************************/
        /* If we're in Passive Listen mode ensure that the external field is
         * still On  */
        if (rfalIsModePassiveListen(gRFAL.mode)) {
            if (!rfalIsExtFieldOn()) {
    ...
    case RFAL_TXRX_STATE_TX_TRANSMIT: /*  PRQA S 2003 # MISRA 16.3 - Intentional
        ...
        if (gRFAL.callbacks.syncTxRx != NULL) {
            /* If set, wait for sync callback to signal sync/trigger
             * transmission */
    ...
    case RFAL_TXRX_STATE_TX_RELOAD_FIFO: /*  PRQA S 2003 # MISRA 16.3 -
        ...
        if ((RFAL_MODE_POLL_NFCV == gRFAL.mode) ||
            ...
            /* Load FIFO with the remaining length or maximum available (which
             * fit on the coding buffer) */
            maxLen =
                (uint16_t)MIN((gRFAL.fifo.bytesTotal - gRFAL.fifo.bytesWritten),
                              gRFAL.fifo.expWL);
...
#endif /* RFAL_FEATURE_NFCV */
        {
            ...
            tmp = MIN((gRFAL.fifo.bytesTotal - gRFAL.fifo.bytesWritten),
    ...
    case RFAL_TXRX_STATE_TX_WAIT_TXE:
        ...
        if ((irqs & ST25R3916_IRQ_MASK_TXE) != 0U) {
            ...
            break; /* Ignore ST25R3916 FIFO WL if total TxLen is already on the
                      FIFO */
        } else {
            gRFAL.TxRx.status = ERR_IO;
            gRFAL.TxRx.state = RFAL_TXRX_STATE_TX_FAIL;
...
rfalTransceiveRx(void)
{
    ...
    case RFAL_TXRX_STATE_RX_WAIT_RXS:
        ...
        if (((irqs & ST25R3916_IRQ_MASK_EOF) != 0U) &&
             ...
             * mute */
            gRFAL.TxRx.status =
                (rfalIsModeActiveComm(gRFAL.mode) ? ERR_TIMEOUT
                                                  : ERR_LINK_LOSS);
            gRFAL.TxRx.state = RFAL_TXRX_STATE_RX_FAIL;
            break;
        }
    ...
    case RFAL_TXRX_STATE_RX_WAIT_RXE: /*  PRQA S 2003 # MISRA 16.3 - Intentional
        ...
        if ((irqs & ST25R3916_IRQ_MASK_RX_REST) != 0U) {
             ...
             * therefore a RXS + RXE should * follow if a good reception is
             * followed within the valid initial timeout                   */
    ...
    case RFAL_TXRX_STATE_RX_ERR_CHECK: /*  PRQA S 2003 # MISRA 16.3 -
        ...
        /* Discard Soft Framing errors in CTS as Correlator does not support
           no_eof */
            ...
            /* Check if the reception ends with an incomplete byte (residual
             * bits) */
            if (rfalFIFOStatusIsIncompleteByte()) {
                gRFAL.TxRx.status = ERR_INCOMPLETE_BYTE;
            }
    ...
    case RFAL_TXRX_STATE_RX_READ_DATA: /*  PRQA S 2003 # MISRA 16.3 -
        ...
        if (((gRFAL.TxRx.ctx.flags & (uint32_t)RFAL_TXRX_FLAGS_CRC_RX_KEEP) ==
            ...
            if ((uint16_t)(gRFAL.fifo.bytesTotal + tmp) > 0U) {
                ...
                /* If the CRC was already placed into rxBuffer (due to WL
                 * interrupt where CRC was already in FIFO Read) cannot remove
                 * it from rxBuf. Can only remove it from rxBufLen not indicate
                 * the presence of CRC    */
...
#if RFAL_FEATURE_NFCV
        /*******************************************************************************/
        /* Decode sub bit stream into payload bits for NFCV, if no error found
         * so far  */
        if (((RFAL_MODE_POLL_NFCV == gRFAL.mode) ||
             (RFAL_MODE_POLL_PICOPASS == gRFAL.mode)) &&
            (gRFAL.TxRx.status == ERR_BUSY)) {
            ...
            if (((ERR_NONE == ret) || (ERR_CRC == ret)) &&
                ...
                ((*gRFAL.nfcvData.origCtx.rxRcvdLen % RFAL_BITS_IN_BYTE) ==
                 0U) &&
                (*gRFAL.nfcvData.origCtx.rxRcvdLen >=
                 rfalConvBytesToBits(RFAL_CRC_LEN))) {
                *gRFAL.nfcvData.origCtx.rxRcvdLen -=
                    (uint16_t)rfalConvBytesToBits(
                        RFAL_CRC_LEN); /* Remove CRC */
            }
#if 0
                /* Debugging code: output the payload bits by writing into the FIFO and subsequent clearing */
    ...
    case RFAL_TXRX_STATE_RX_READ_FIFO:
        ...
        /*******************************************************************************/
        /* If the bytes already read were not the full FIFO WL, dump the
         * remaining     * FIFO so that ST25R391x can continue with reception */
        if (aux < tmp) {
    ...
    case RFAL_TXRX_STATE_RX_WAIT_EOF:
        ...
        if ((irqs & ST25R3916_IRQ_MASK_CAT) != 0U) {
            ...
            gRFAL.TxRx.state =
                ((gRFAL.TxRx.status != ERR_BUSY) ? RFAL_TXRX_STATE_RX_FAIL
...
/*******************************************************************************/
ReturnCode
rfalISO14443ATransceiveShortFrame(rfal14443AShortFrameCmd txCmd, uint8_t *rxBuf,
                                  uint8_t rxBufLen, uint16_t *rxRcvdLen,
                                  uint32_t fwt)
{
    ReturnCode ret;
    uint8_t directCmd;
    ...
    st25r3916SetNoResponseTime(rfalConv1fcTo64fc(
        MIN((fwt + RFAL_FWT_ADJUSTMENT + RFAL_FWT_A_ADJUSTMENT),
    ...
    if (gRFAL.timings.FDTListen != RFAL_TIMING_NONE) {
        ...
        st25r3916WriteRegister(
            ST25R3916_REG_MASK_RX_TIMER,
            (uint8_t)rfalConv1fcTo64fc(
                ((RFAL_FDT_LISTEN_MRT_ADJUSTMENT +
                  RFAL_FDT_LISTEN_A_ADJUSTMENT) > gRFAL.timings.FDTListen)
                    ? RFAL_ST25R3916_MRT_MIN_1FC
     ...
     * report parity error, Note2 of the register */
    ...
    if (st25r3916WaitForInterruptsTimed(
            ...
            (uint16_t)MAX(rfalConv1fcToMs(fwt),
...
/*******************************************************************************/
ReturnCode
rfalISO14443ATransceiveAnticollisionFrame(uint8_t *buf, uint8_t *bytesToSend,
                                          uint8_t *bitsToSend,
                                          uint16_t *rxLength, uint32_t fwt)
{
    ReturnCode ret;

    EXIT_ON_ERR(ret, rfalISO14443AStartTransceiveAnticollisionFrame(
...
/*******************************************************************************/
ReturnCode
rfalISO14443AStartTransceiveAnticollisionFrame(uint8_t *buf,
                                               uint8_t *bytesToSend,
                                               uint8_t *bitsToSend,
                                               uint16_t *rxLength, uint32_t fwt)
{
    ...
    /* Disable Automatic Gain Control (AGC) for better detection of collisions
     * if using Coherent Receiver */
    ctx.flags |=
        (st25r3916CheckReg(ST25R3916_REG_AUX, ST25R3916_REG_AUX_dis_corr,
...
/*******************************************************************************/
ReturnCode
rfalISO14443AGetTransceiveAnticollisionFrameStatus(void)
{
    ReturnCode ret;
    uint8_t collData;
    ...
    rfalSetAnalogConfig((RFAL_ANALOG_CONFIG_POLL |
                         RFAL_ANALOG_CONFIG_TECH_NFCA |
...
/*******************************************************************************/
ReturnCode
rfalISO15693TransceiveAnticollisionFrame(uint8_t *txBuf, uint8_t txBufLen,
...
/*******************************************************************************/
ReturnCode
rfalISO15693TransceiveEOFAnticollision(uint8_t *rxBuf, uint8_t rxBufLen,
                                       uint16_t *actLen)
{
    uint8_t dummy;
...
/*******************************************************************************/
ReturnCode
rfalISO15693TransceiveEOF(uint8_t *rxBuf, uint8_t rxBufLen, uint16_t *actLen)
{
    ReturnCode ret;
    uint8_t dummy;
...
/*******************************************************************************/
ReturnCode
rfalFeliCaPoll(rfalFeliCaPollSlots slots, uint16_t sysCode, uint8_t reqCode,
               rfalFeliCaPollRes *pollResList, uint8_t pollResListSize,
               uint8_t *devicesDetected, uint8_t *collisionsDetected)
{
    ReturnCode ret;

    EXIT_ON_ERR(ret, rfalStartFeliCaPoll(slots, sysCode, reqCode, pollResList,
                                         pollResListSize, devicesDetected,
                                         collisionsDetected));
...
/*******************************************************************************/
ReturnCode
rfalStartFeliCaPoll(rfalFeliCaPollSlots slots, uint16_t sysCode,
                    ...
                    uint8_t *collisionsDetected)
{
     ...
     *                       512 PICC process time + (n * 256 Time Slot
     * duration)  */
...
rfalGetFeliCaPollStatus(void)
{
    ...
    if (ret != ERR_TIMEOUT) {
        ...
        if (ret == ERR_NONE) {
            ...
            gRFAL.TxRx.ctx.rxBuf =
                (uint8_t *)
                    gRFAL.nfcfData.pollResponses[gRFAL.nfcfData.devDetected];
        }
    ...
    if ((gRFAL.nfcfData.pollResList != NULL) &&
        ...
        ST_MEMCPY(gRFAL.nfcfData.pollResList, gRFAL.nfcfData.pollResponses,
                  (RFAL_FELICA_POLL_RES_LEN *
                   (uint32_t)MIN(gRFAL.nfcfData.pollResListSize,
                                 gRFAL.nfcfData.devDetected)));
    }
    ...
    return (((gRFAL.nfcfData.colDetected != 0U) ||
             (gRFAL.nfcfData.devDetected != 0U))
                ? ERR_NONE
                : ret);
}
...
/*******************************************************************************/
ReturnCode
rfalListenStart(uint32_t lmMask, const rfalLmConfPA *confA,
                ...
                uint8_t *rxBuf, uint16_t rxBufLen, uint16_t *rxLen)
{
    t_rfalPTMem PTMem; /*  PRQA S 0759 # MISRA 19.2 - Allocating Union where
                          members are of the same type, just different names.
                          Thus no problem can occur. */
    uint8_t *pPTMem;
    uint8_t autoResp;
    ...
    if ((lmMask & RFAL_LM_MASK_NFCF) != 0U) {
        ...
        /*******************************************************************************/
        /* Write 24 TSN "Random" Numbers at first initialization and let it
         * rollover   */
        if (!gRFAL.Lm.iniFlag) {
        ...
        /* In CE NFC-F any data without error will be passed to FIFO, to support
         * CUP */
        gRFAL.Lm.mdIrqs |=
            (ST25R3916_IRQ_MASK_WU_F | ST25R3916_IRQ_MASK_RXE_PTA |
    ...
    if ((lmMask & RFAL_LM_MASK_ACTIVE_P2P) != 0U) {
        ...
        /* Set Target mode, Bit Rate detection and Automatic Response RF
         * Collision Avoidance */
        gRFAL.Lm.mdReg |=
            (ST25R3916_REG_MODE_targ_targ | ST25R3916_REG_MODE_om3 |
    ...
    if ((gRFAL.Lm.mdReg & ST25R3916_REG_MODE_targ) ==
        ...
        *gRFAL.Lm.rxLen = 0;
        ...
        /* On Bit Rate Detection Mode ST25R391x will filter incoming frames
         * during MRT time starting on External Field On event, use 512/fc steps
         */
        st25r3916SetRegisterBits(ST25R3916_REG_TIMER_EMV_CONTROL,
        ...
        /* Restore default settings on NFCIP1 mode, Receiving parity + CRC bits
         * and manual Tx Parity*/
        st25r3916ClrRegisterBits(ST25R3916_REG_ISO14443A_NFC,
                                 (ST25R3916_REG_ISO14443A_NFC_no_tx_par |
                                  ST25R3916_REG_ISO14443A_NFC_no_rx_par |
                                  ST25R3916_REG_ISO14443A_NFC_nfc_f0));
        ...
        /* Not on SetState(POWER OFF) as otherwise would be applied on every
         * Field Event */
        rfalSetAnalogConfig(
...
rfalRunListenModeWorker(void)
{
    ...
    case RFAL_LM_STATE_IDLE: /*  PRQA S 2003 # MISRA 16.3 - Intentional fall
        ...
        if ((irqs & ST25R3916_IRQ_MASK_NFCT) != 0U) {
            ...
            gRFAL.Lm.brDetected =
                (rfalBitRate)(newBr); /* PRQA S 4342 # MISRA 10.5 - Guaranteed
                                         that no invalid enum values may be
                                         created. See also
                                         equalityGuard_RFAL_BR_106 ff.*/
        }
        ...
        if (((irqs & ST25R3916_IRQ_MASK_EOF) != 0U) && (!gRFAL.Lm.dataFlag)) {
            ...
            /* may not be able to emit RF carrier depending on the pt_res
             * setting          */
            /* Preemptively enter AP2P before FIFO retrieval and protocol
             * checking         */
            ...
            *gRFAL.Lm.rxLen = st25r3916GetNumFIFOBytes();
            ...
            if ((*gRFAL.Lm.rxLen > RFAL_CRC_LEN) &&
                ...
                if (rfalCrcCalculateCcitt(RFAL_ISO14443A_CRC_INTVAL,
                                          gRFAL.Lm.rxBuf,
                                          *gRFAL.Lm.rxLen) != 0U) {
                    st25r3916ExecuteCommand(ST25R3916_CMD_CLEAR_FIFO);
                    st25r3916ExecuteCommand(ST25R3916_CMD_UNMASK_RECEIVE_DATA);
            ...
            *gRFAL.Lm.rxLen -=
                ((*gRFAL.Lm.rxLen > RFAL_CRC_LEN) ? RFAL_CRC_LEN
                                                  : *gRFAL.Lm.rxLen);
            *gRFAL.Lm.rxLen = (uint16_t)rfalConvBytesToBits(*gRFAL.Lm.rxLen);
            ...
            if (((gRFAL.Lm.mdMask & RFAL_LM_MASK_NFCA) != 0U) &&
                (gRFAL.Lm.brDetected == RFAL_BR_106)) {
                st25r3916ReadRegister(ST25R3916_REG_PASSIVE_TARGET_STATUS,
                                      &tmp);
    ...
    case RFAL_LM_STATE_SLEEP_AF:
        ...
        if ((irqs & ST25R3916_IRQ_MASK_EOF) != 0U) {
            ...
            *gRFAL.Lm.rxLen = 0;
            ...
            /* REMARK: In order to support CUP or proprietary frames, handling
             * could be added here */
        } else if (((irqs & ST25R3916_IRQ_MASK_RXE_PTA) != 0U) &&
                   (gRFAL.Lm.brDetected != RFAL_BR_KEEP)) {
            if (((gRFAL.Lm.mdMask & RFAL_LM_MASK_NFCA) != 0U) &&
...
rfalListenStop(void)
{
    ...
    st25r3916GetInterrupt((ST25R3916_IRQ_MASK_RXE_PTA |
                           ST25R3916_IRQ_MASK_WU_F | ST25R3916_IRQ_MASK_WU_A |
...
/*******************************************************************************/
ReturnCode
rfalListenSleepStart(rfalLmState sleepSt, uint8_t *rxBuf, uint16_t rxBufLen,
                     uint16_t *rxLen)
{
...
/*******************************************************************************/
rfalLmState
rfalListenGetState(bool *dataFlag, rfalBitRate *lastBR)
{
...
/*******************************************************************************/
ReturnCode
rfalListenSetState(rfalLmState newSt)
{
    ReturnCode ret;
    ...
    do {
        ...
        case RFAL_LM_STATE_POWER_OFF:
            ...
            if ((gRFAL.Lm.mdMask & RFAL_LM_MASK_NFCA) != 0U) {
                ...
                /* Prepares the NFCIP-1 Passive target logic to wait in the
                 * Sense/Idle state */
            ...
            if ((gRFAL.Lm.mdMask & RFAL_LM_MASK_ACTIVE_P2P) != 0U) {
                /* Ensure automatic response RF Collision Avoidance is back to
                 * only after Rx */
                st25r3916ChangeRegisterBits(ST25R3916_REG_MODE,
                                            ST25R3916_REG_MODE_nfc_ar_mask,
                                            ST25R3916_REG_MODE_nfc_ar_auto_rx);

                /* Ensure that our field is Off, as automatic response RF
                 * Collision Avoidance may have been triggered */
                st25r3916TxOff();
            }
        ...
        case RFAL_LM_STATE_IDLE:
            ...
            /*******************************************************************************/
            /* In Active P2P the Initiator may:  Turn its field On;  LM goes
             * into IDLE state; Initiator sends an unexpected frame raising a
             * Protocol error; Initiator turns its field Off and ST25R3916
             * performs the automatic RF Collision Avoidance keeping our field
             * On; upon a Protocol error upper layer sets again the state to
             * IDLE to clear dataFlag and wait for next data.
             *
            ...
            if ((gRFAL.Lm.mdMask & RFAL_LM_MASK_ACTIVE_P2P) != 0U) {
                ...
                if (!st25r3916IsExtFieldOn()) {
                    reSetState = true;
                    newState =
                        RFAL_LM_STATE_POWER_OFF; /* Set POWER_OFF state */
                }
            }
            /*******************************************************************************/

            /* If we are in ACTIVE_A, reEnable Listen for A before going to
        ...
        case RFAL_LM_STATE_READY_F:
            ...
            /*******************************************************************************/
            /* If we're coming from BitRate detection mode, the Bit Rate
             * Definition reg still has the last bit rate used. If a frame is
             * received between setting the mode to Listen NFCA and setting Bit
             * Rate Definition reg, it will raise a framing error. Set the
             * bitrate immediately, and then the normal SetMode procedure */
...
/*******************************************************************************/
ReturnCode
rfalWakeUpModeStart(const rfalWakeUpConfig *config)
{
    uint8_t aux;
    uint8_t reg;
    uint32_t irqs;
    ...
    reg |= (uint8_t)(((uint8_t)gRFAL.wum.cfg.period <
    ...
    if (gRFAL.wum.cfg.swTagDetect) {
        ...
        if (gRFAL.wum.cfg.indAmp.enabled) {
            ...
            aux |= (uint8_t)(gRFAL.wum.cfg.indAmp.aaInclMeas
                                 ...
                                 : 0x00U);
            aux |=
                (uint8_t)(((uint8_t)gRFAL.wum.cfg.indAmp.aaWeight
        ...
        if (gRFAL.wum.cfg.indPha.enabled) {
            ...
            aux |= (uint8_t)(gRFAL.wum.cfg.indPha.aaInclMeas
                                 ...
                                 : 0x00U);
            aux |=
                (uint8_t)(((uint8_t)gRFAL.wum.cfg.indPha.aaWeight
        ...
        if (gRFAL.wum.cfg.cap.enabled) {
            ...
            /*******************************************************************************/
            aux =
                (uint8_t)((gRFAL.wum.cfg.cap.delta)
...
/*******************************************************************************/
ReturnCode
rfalWakeUpModeGetInfo(bool force, rfalWakeUpInfo *info)
{
    uint8_t aux;
    ...
    if (gRFAL.wum.cfg.indAmp.enabled) {
        ...
        if ((force || (info->irqWut) || (gRFAL.wum.info.indAmp.irqWu)) &&
            ...
            if (gRFAL.wum.cfg.indAmp.autoAvg) {
                st25r3916ReadRegister(ST25R3916_REG_AMPLITUDE_MEASURE_AA_RESULT,
                                      &aux);
            ...
            gRFAL.wum.info.indAmp.lastMeas =
                info->indAmp
                    .lastMeas; /* Store last value for subsequenct calls */
        }
    ...
    if (gRFAL.wum.cfg.indPha.enabled) {
        ...
        if ((force || (info->irqWut) || (gRFAL.wum.info.indPha.irqWu)) &&
            (!gRFAL.wum.cfg.swTagDetect)) {
            if (gRFAL.wum.cfg.indPha.autoAvg) {
                st25r3916ReadRegister(ST25R3916_REG_PHASE_MEASURE_AA_RESULT,
                                      &aux);
        ...
        if (force || (info->irqWut) || (gRFAL.wum.info.cap.irqWu)) {
            ...
            if (gRFAL.wum.cfg.cap.autoAvg) {
                st25r3916ReadRegister(
                    ST25R3916_REG_CAPACITANCE_MEASURE_AA_RESULT, &aux);
        ...
        if ((irqs & ST25R3916_IRQ_MASK_WT) != 0U) {
            ...
            if (gRFAL.wum.cfg.swTagDetect) {
                ...
                if (gRFAL.wum.cfg.indAmp.enabled) {
                    ...
                    delta |=
                        rfalAddFracTDFormat(gRFAL.wum.cfg.indAmp.fracDelta);
                ...
                if (gRFAL.wum.cfg.indPha.enabled) {
                    ...
                    delta |=
                        rfalAddFracTDFormat(gRFAL.wum.cfg.indPha.fracDelta);
...
/*******************************************************************************/
ReturnCode
rfalChipWriteReg(uint16_t reg, const uint8_t *values, uint8_t len)
{
...
/*******************************************************************************/
ReturnCode
rfalChipReadReg(uint16_t reg, uint8_t *values, uint8_t len)
{
    if (!st25r3916IsRegValid((uint8_t)reg)) {
        return ERR_PARAM;
    }
...
/*******************************************************************************/
ReturnCode
rfalChipWriteTestReg(uint16_t reg, uint8_t value)
{
...
/*******************************************************************************/
ReturnCode
rfalChipReadTestReg(uint16_t reg, uint8_t *value)
{
...
/*******************************************************************************/
ReturnCode
rfalChipChangeRegBits(uint16_t reg, uint8_t valueMask, uint8_t value)
{
    if (!st25r3916IsRegValid((uint8_t)reg)) {
        return ERR_PARAM;
    }
...
/*******************************************************************************/
ReturnCode
rfalChipChangeTestRegBits(uint16_t reg, uint8_t valueMask, uint8_t value)
{
...
/*******************************************************************************/
ReturnCode
rfalChipMeasureAmplitude(uint8_t *result)
{
    ReturnCode err;
    ...
    /* Set values as per defaults of DS. These regs/bits influence receiver
     * chain and change amplitude */
...
/*******************************************************************************/
ReturnCode
rfalChipMeasurePhase(uint8_t *result)
{
...
/*******************************************************************************/
ReturnCode
rfalChipMeasureCapacitance(uint8_t *result)
{
...
/*******************************************************************************/
ReturnCode
rfalChipMeasurePowerSupply(uint8_t param, uint8_t *result)
{
