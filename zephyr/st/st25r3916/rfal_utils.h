...
#define REVERSE_BYTES(pData, nDataSize)                                        \
    {                                                                          \
        unsigned char swap, *lo = ((unsigned char *)(pData)),                  \
                            *hi = ((unsigned char *)(pData)) + (nDataSize)-1;  \
        while (lo < hi) {                                                      \
            swap = *lo;                                                        \
            *lo++ = *hi;                                                       \
            *hi-- = swap;                                                      \
...
#ifdef __CSMC__
/* STM8 COSMIC */
#define ST_MEMMOVE(s1, s2, n)                                                  \
    memmove(s1, s2, n) /*!< map memmove to string library code */
static inline void *
ST_MEMCPY(void *s1, const void *s2, uint32_t n)
{
...
#define ST_MEMSET(s1, c, n)                                                    \
    memset(s1, (char)(c), n) /*!< map memset to string library code  */
static inline int32_t
ST_BYTECMP(void *s1, const void *s2, uint32_t n)
{
